...
if($_POST['type'] == 'delivered'){
    ...
    $delive++;
    $where = " where o.iStatusCode in (7,8,9,11,12) AND o.tOrderRequestDate BETWEEN '$Startdate' AND '$Enddate'";
}
...
if(count($DBProcessingOrders) > 0) {
    foreach ($DBProcessingOrders as $val) {
        ...
        */
        ...
        if($val['iStatusCode'] == '8' || $val['iStatusCode'] == '11' || $val['iStatusCode'] == '12'){
            $status = 'CL';
            $statuscolor = '#a3a3c2';
        }
        $cls = '';
...
$CANCOrderQ = "SELECT COUNT(`iOrderId`) AS 'tcount' FROM `orders` where iStatusCode in(7,8,9,11,12) AND tOrderRequestDate BETWEEN '$Startdate' AND '$Enddate';";
