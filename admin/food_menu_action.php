...
if($count_all > 0) {
  for($i=0;$i<$count_all;$i++) {
    ...
    /*array_push($vMenuDesc_store ,$vValue_desc);   
...
if($id != "") {
  ...
  for($i=0;$i<count($vMenu_store);$i++) {   
    ...
    if ($action == "Add") {
        $_SESSION['success'] = '1';
        $_SESSION['var_msg'] = 'Item Category Insert Successfully.';
    } else {
        $_SESSION['success'] = '1';
        $_SESSION['var_msg'] = 'Item Category Updated Successfully.';
    }
...
$sql = "SELECT * FROM `company` where eStatus !='Deleted' ORDER BY `vCompany`";
