...
if(isset($_GET['del']) && $_GET['del'] != ''){
	...
	$usAr = 'iUserId';
	$sql = "SELECT * FROM `pushnotification_log` where iPushnotificationId = $id and status != 'deleted' and etype='Offer' ";
...
$sql = "SELECT count(pnl.iPushnotificationId) as Total FROM `pushnotification_log` as pnl left join register_user as ru on pnl.iUserId = ru.iUserId where pnl.status != 'deleted' and pnl.etype ='Offer' $ssql";
...
if ($page <= 0)
    $page = 1;
//Pagination End
$sql = "SELECT pnl.iPushnotificationId,pnl.tMessage,pnl.vTitle, pnl.dDateTime,concat(ru.vName,' ', ru.vLastName) as name FROM `pushnotification_log` as pnl left join register_user as ru on pnl.iUserId = ru.iUserId where pnl.status != 'deleted' and pnl.etype ='Offer'$ssql $ord LIMIT $start, $per_page";
//echo $sql;die;
                                            ...
                                            <table class="table table-striped table-bordered table-hover">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th width="20%"><a href="javascript:void(0);" onClick="Redirect(1,<?php if($sortby == '2'){ echo $order; }else { ?>0<?php } ?>)">Title <?php if ($sortby == 2) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>
