...
if(isset($_GET['method']) && $_GET['method'] == 'checkbox'){
	...
	$_SESSION['success'] = '1';
	if($status == 'active'){
		$_SESSION['var_msg'] = 'News activated Successfully.';
	}elseif($status == 'inactive'){
		$_SESSION['var_msg'] = 'News inactivated Successfully.';
	}else{
		$_SESSION['var_msg'] = 'News '.$status.' Successfully.';
	}
...
if(isset($_GET['del']) && $_GET['del'] != ''){
	...
	$usAr = 'iUserId';
	$sql = "SELECT * FROM `pushnotification_log` where iPushnotificationId = $id and status != 'deleted' and etype='News'";
	...
	$sql = "update 
     set status = 'deleted' where iPushnotificationId = $id";
	$db_customnotif = $obj->MySQLSelect($sql);
	$_SESSION['success'] = '1';
	$_SESSION['var_msg'] = 'News deleted Successfully.';
...
if($sortby == 1){
  if($order == 0)
  $ord = " ORDER BY pnl.vTitle ASC";
  else
  $ord = " ORDER BY pnl.vTitle DESC";
}
...
if($keyword != ''){
    if($option != '') {
        ...
        $ssql.= " AND (concat(ru.vName,' ',ru.vLastName) LIKE '%".$generalobjAdmin->clean($keyword)."%' OR pnl.vTitle LIKE '%".$generalobjAdmin->clean($keyword)."%')";
    }
}
...
$sql = "SELECT count(pnl.iPushnotificationId) as Total FROM `pushnotification_log` as pnl left join register_user as ru on pnl.iUserId = ru.iUserId where pnl.status != 'deleted' AND pnl.etype = 'News' $ssql";
...
if ($page <= 0)
    $page = 1;
//Pagination End
$sql = "SELECT pnl.iPushnotificationId, pnl.tMessage,pnl.vTitle ,pnl.dDateTime,concat(ru.vName,' ', ru.vLastName) as name FROM `pushnotification_log` as pnl left join register_user as ru on pnl.iUserId = ru.iUserId where pnl.status != 'deleted' and pnl.etype='News' $ssql $ord LIMIT $start, $per_page";
//echo $sql;die;
...
<html lang="en">
    ...
    <head>
        <meta charset="UTF-8" />
        <title><?=$SITE_NAME?> | <?php echo $langage_lbl_admin['LBL_NEWS_CAPTION'];?></title>
            ...
            <div id="content">
                <div class="inner">
                    <div id="add-hide-show-div">
                        <div class="row">
                            <div class="col-lg-12">
                                <h2><?php echo $langage_lbl_admin['LBL_NEWS_CAPTION'];?></h2>
                              ...
                              <tbody>
                                <tr>
                                    ...
                                    <td width="8%" class=" padding-right10"><select name="option" id="option" class="form-control">
                                          ...
                                          <option value="pnl.vTitle" <?php if ($option == 'pnl.vTitle') {echo "selected"; } ?> >Title</option>
                                    </select>
                                    </td>
                                    ...
                                    <td width="30%"><a href="send_news_notification_withimage.php" class="add-btn">Add <?php echo $langage_lbl_admin['LBL_NEWS_CAPTION'];?></a></td>
                                </tr>
                              </tbody>
                        </table>
                                                                   ...
                                                                   <div class="social show-moreOptions openPops_<?= $data_drv[$i]['iAdminId']; ?>">
                                                                        <ul>
                                                                            <li class="entypo-twitter" data-network="twitter">
																				<a href="send_news_notification_withimage.php?id=<?= base64_encode($data_drv[$i]['iPushnotificationId']); ?>" data-toggle="tooltip" title="Edit">
																					<img src="img/edit-icon.png" alt="Edit">
																				</a>
																			</li>
                                                                            
                                                                            <li class="entypo-gplus" data-network="gplus">
																				<a href="news.php?del=<?php echo base64_encode($data_drv[$i]['iPushnotificationId']); ?>"  onclick="return confirm('Are you sure?')"  data-toggle="tooltip" title="Delete">
                    ...
                    <div class="admin-notes">
                            ...
                            <ul>
                                    <li>
                                            <?php echo $langage_lbl_admin['LBL_NEWS_CAPTION'];?> module will list all <?php echo $langage_lbl_admin['LBL_NEWS_CAPTION'];?> on this page.
                                    </li>
                                    <li>
                                            Administrator can Activate / Deactivate / Delete any <?php echo $langage_lbl_admin['LBL_NEWS_CAPTION'];?>
                                    </li>
                            </ul>
                    </div>
                    </div>
...
<form name="pageForm" id="pageForm" action="news.php" method="post" >
