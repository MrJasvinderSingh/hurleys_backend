...
$script = 'DeliveryLocation';
    ...
    $tbl_name       = 'location_master_driver';
    //$script           = "Settings";
    ...
    if($hdn_del_id != ''){
        ...
        header("Location:deliverylocation.php?success=2");exit;
    }
    }
...
if($sortby == 4){
  if($order == 0)
  $ord = " ORDER BY lm.mon ASC";
  else
  $ord = " ORDER BY lm.mon DESC";
} 
if($sortby == 5){
  if($order == 0)
  $ord = " ORDER BY lm.mat ASC";
  else
  $ord = " ORDER BY lm.mat DESC";
}
//End Sorting
...
$mat = isset($_REQUEST['mat']) ? $_REQUEST['mat'] : "";
$ssql = '';
if($keyword != ''){
    if($option != '') {
...
$sql = "SELECT COUNT(lm.iLocationId) AS Total FROM location_master_driver as lm LEFT JOIN country as c on c.iCountryId=lm.iCountryId WHERE lm.eStatus != 'Deleted' $ssql";
...
if ($page <= 0)
    $page = 1;
//Pagination End
 $sql = "SELECT lm.*,c.vCountry FROM location_master_driver as lm LEFT JOIN country as c on c.iCountryId=lm.iCountryId WHERE lm.eStatus != 'Deleted' $ssql $ord LIMIT $start, $per_page ";
            ...
            <div id="content">
                <div class="inner">
                    <div id="add-hide-show-div">
                        <div class="row">
                            <div class="col-lg-12">
                                <h2>Delivery Location For Drivers</h2>
                                ...
                                <tr>
                                    ...
                                    <td width="10%" class=" padding-right10"><select name="option" id="option" class="form-control">
                                          ...
                                          <option value="lm.eStatus" <?php if ($option == 'lm.eStatus') {echo "selected"; } ?> >Status</option>
                                           <option value="lm.mon" <?php if ($option == "lm.mon") { echo "selected"; } ?> >Maximum order number</option>
                                           <option value="lm.mat" <?php if ($option == "lm.mat") { echo "selected"; } ?> >Maximum Allocation time</option>
                                    </select>
                                    </td>
                                    ...
                                    <td width="12%">
                                      ...
                                      <input type="button" value="Reset" class="btnalt button11" onClick="window.location.href='deliverylocation.php'"/>
                                    </td>
                                    <td width="30%"><a class="add-btn" href="deliverylocation_action.php" style="text-align: center;">Add Delivery Location</a></td>
                                </tr>
                              </tbody>
                        </table>
                                            ...
                                            <table class="table table-striped table-bordered table-hover">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th align="center"><a href="javascript:void(0);" onClick="Redirect(4,<?php if($sortby == '4'){ echo $order; }else { ?>0<?php } ?>)">Max Order Number <?php if ($sortby == 4) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>
                                                        <th align="center"><a href="javascript:void(0);" onClick="Redirect(5,<?php if($sortby == '5'){ echo $order; }else { ?>0<?php } ?>)">Max Allocation Time <?php if ($sortby == 5) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr class="gradeA">
                                                        ...
                                                        <td align="center" style="text-align:center;">
                                                             ...
                                                             <td><?= $data_drv[$i]['mon']; ?></td>
                                                              <td><?= $data_drv[$i]['mat']; ?></td>
                                                            <td align="center" style="text-align:center;" class="action-btn001">
                                                                <div class="share-button openHoverAction-class" style="display: block;">
                                                                    <label class="entypo-export"><span><img src="images/settings-icon.png" alt=""></span></label>
                                                                    <div class="social show-moreOptions openPops_<?= $data_drv[$i]['iLocationId']; ?>">
                                                                        <ul>
                                                                            <li class="entypo-twitter" data-network="twitter"><a href="deliverylocation_action.php?id=<?= $data_drv[$i]['iLocationId']; ?>" data-toggle="tooltip" title="Edit">
...
<form name="pageForm" id="pageForm" action="action/deliverylocation.php" method="post" >
