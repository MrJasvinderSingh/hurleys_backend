...
if ($method == 'delete' && $iUserId != '') {
	if(SITE_TYPE !='Demo'){
		...
		$check_user = $obj->MySQLSelect($sql);
		...
		$sql = "SELECT * from [dbo].[LOYAL_CUST] where EMAIL_ADDR='".$check_user[0]['vEmail']."'";
		...
		if($row1){
			$sql = "DELETE from [dbo].[LOYAL_CUST] where EMAIL_ADDR='".$check_user[0]['vEmail']."'";
			$stmt = sqlsrv_query( $conn, $sql);
			if( $stmt === false ) {
				//$returnArr['Action'] = "0";
				//$returnArr['message'] = "LBL_SERVER_COMM_ERROR";
		...
		$query = "DELETE from register_user WHERE iUserId = '" . $iUserId . "'";
		$obj->sql_query($query);
		$_SESSION['success'] = '1';
