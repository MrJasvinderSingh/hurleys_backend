...
if ($method == 'delete' && $iLocationId != '') {
	if(SITE_TYPE !='Demo'){
            $query1 = "SELECT * FROM militarylocations WHERE id = '" . $iLocationId . "'";
            $checklocation = $obj->MySQLSelect($query1);

           

            $query = "DELETE FROM militarylocations WHERE id = '" . $iLocationId . "'";
            $obj->sql_query($query);
            $_SESSION['success'] = '1';
...
if ($iLocationId != '' && $status != '') {
	if(SITE_TYPE !='Demo') {
            ...
            $query = "UPDATE militarylocations SET eStatus = '" . $status . "' WHERE id = '" . $iLocationId . "'";
            $obj->sql_query($query);
            $_SESSION['success'] = '1';
            if($status == 'Active') {
        ...
        header("Location:".$tconfig["tsite_url_main_admin"]."militarylocations.php?".$parameters);
        exit;
}
...
if($checkbox != "" && $statusVal != "") {
	if(SITE_TYPE !='Demo'){
        if($statusVal == "Deleted") {
            $query = "DELETE FROM militarylocations WHERE id IN (" . $checkbox . ")";
            $obj->sql_query($query);
            $_SESSION['success'] = '1';
            $_SESSION['var_msg'] = 'Location(s) deleted successfully.';
        } else {
            $query = "UPDATE militarylocations SET eStatus = '" . $statusVal . "' WHERE id IN (" . $checkbox . ")";
            $obj->sql_query($query);
            $_SESSION['success'] = '1';
