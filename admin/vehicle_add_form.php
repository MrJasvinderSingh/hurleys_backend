			...
			if($id != "" && $db_mdl[0]['eStatus'] != $eStatus) {
				if($SEND_TAXI_EMAIL_ON_CHANGE == 'Yes') {
					$sql23 = "SELECT m.vMake, md.vTitle,rd.vEmail, rd.vName, rd.vLastName
						FROM driver_vehicle dv, register_driver rd, make m, model md WHERE dv.eStatus != 'Deleted' AND dv.iDriverId = rd.iDriverId AND dv.iModelId = md.iModelId AND dv.iMakeId = m.iMakeId AND dv.iDriverVehicleId = '".$id."'";
		...
		<div id="wrap">
			...
			<div id="content">
				<div class="inner">
					<div class="row">
						<div class="col-lg-12">
							<h2><?// = $action." ".$langage_lbl_admin['LBL_TEXI_ADMIN'];?> Assign Vehicle</h2>
					...
					<div class="body-div">
						<div class="form-group">
							...
							<div class="row">
									...
									<div class="col-lg-6">
										<!-- <div id="driver"> -->
										<select name = "vCarType[]" id="vehicleTypes001" class="form-control">
											<option value=""><?php echo "Choose vehicle type";?> </option>
											<?php
                                                                
                                                              //  echo $Vehicle_type_name;
                                                               // print_r($vehicle_type_data);
									foreach ($vehicle_type_data as $key => $value) { ?>
                                                                                        <?php $vname= $value['vVehicleType_'.$default_lang]; ?>
                                                                                        <option value="<?= $value['iVehicleTypeId'] ?>" <?php if(in_array($value['iVehicleTypeId'],$vCarTyp)){ ?>selected<?php } ?>><?php echo $vname;?> - <?php if(($value['iLocationid'] == "-1")) { echo "( All Locations )"; } ?></option>
							...
							<?php } ?>	
								<div class="row">
                                    <div class="col-lg-12">
                                        <input type="submit" class="btn btn-default" name="submit" id="submit" value="Assign Vehicle<?/*= $action; ?> <?php echo $langage_lbl_admin['LBL_TEXI_ADMIN']; */?>" >
...
function get_vehicleType(iDriverId,selected) {
	...
	var request = $.ajax({
		type: "POST",
		url: '../ajax_find_vehicleTypeSelect.php',
		data: "iDriverId=" + iDriverId +"&selected="+selected,
                dataType: 'json',
		success: function (data) {
                   // alert(data);
                   if(data){
                  $('#vehicleTypes001').find('option').not(':first').remove();
                    $.each(data, function(i, value) {
                        if(value.selected == 'yes')
                        {
    $('#vehicleTypes001').append($('<option>').text(value.text).attr('value', value.val).attr('selected','selected' ));
                        }
                        else
                        {
                          $('#vehicleTypes001').append($('<option>').text(value.text).attr('value', value.val));  
                        }
        });
