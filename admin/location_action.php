<?php
	...
	if(isset($_POST['submit'])) {
		...
		if(empty($tLongitude) || empty($tLatitude)) {
			...
			if ($action == "Add") {
	            $_SESSION['success'] = '1';
	            $_SESSION['var_msg'] = 'Location inserted successfully.';
	        } else {
	            $_SESSION['success'] = '1';
	            $_SESSION['var_msg'] = 'Location Updated Successfully.';
	        }
												...
												<select class="form-control" name ="eFor" id="eFor" required="required">
												   <option value="">Select Location For</option>
												    <option value ="Restrict" <?if($eFor=='Restrict'){?>selected<? } ?>><?php echo $langage_lbl_admin['LBL_LOCATION_AREA_RESTRICTION'];?></option> 
                                                                                                    <option value ="VehicleType" <?if($eFor=='VehicleType'){?>selected<? } ?> ><?php echo $langage_lbl_admin['LBL_VEHICLE_TYPE_SMALL_TXT'];?></option>
												   	  ...
												   	  <option value ="UserDeliveryCharge" <?if($eFor=='UserDeliveryCharge'){?>selected<? } ?> ><?php echo $langage_lbl_admin['LBL_LOCATION_USER_DELIVERY_CHARGE'];?></option>
                                                                                                          <option value ="DeliverySector" <?if($eFor=='DeliverySector'){?>selected<? } ?> >Delivery Sector</option>
