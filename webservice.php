...
$inwebservice = "1";
//ini_set('display_errors', 1); ini_set('display_startup_errors', 1); error_reporting(E_ERROR | E_WARNING | E_PARSE);
...
$connMsSql = sqlsrv_connect($server, array("database" => 'FrontOff', "UID" => 'HurleyAccess', "PWD" => 'yQs#M6=V'));
//$connMsSql = sqlsrv_connect($server, array("database" => 'eatcayman_new', "UID" => 'root', "PWD" => ''));
//print_r($connMsSqls);print_r( sqlsrv_errors(), true);
if (!$connMsSql) {
...
/* general fucntions */
if ($type != "generalConfigData" && $type != "signIn" && $type != "UpdateUserAddressDetails" && $type !="DisplayUserAddressNew" && $type != "updateVirtualCard" && $type != "getVirtualCard" && $type != "updateDefaultAddress" && $type != "isUserExist" && $type != "getCardDetails" && $type != "checkUserStatus" && $type != "signup" && $type != "LoginWithFB" && $type != "sendVerificationSMS" && $type != "countryList" && $type != "changelanguagelabel" && $type != "requestResetPassword" && $type != "ResetPassword" && $type != "verify_otp_reset_password" && $type != "reset_password_after_verification" && $type != "updateUserProfileDetail" && $type != "UpdateLanguageLabelsValue" && $type != "staticPage" && $type != "sendContactQuery" && $type != "loadAvailableRestaurants" && $type != "getCuisineList" && $type != "loadSearchRestaurants" && $type != "GetRestaurantDetails" && $type != "signup_company" && $type != "GetItemOptionAddonDetails" && $type != "getHomeBanners" && $type != "getBanners" && $type != "getServiceCategories" && $type != "CheckOutOrderEstimateDetails" && $type != "getFAQ" && $type != "getUserLanguagesAsPerServiceType" && $type != "findmilitarybase" && $type != "findmilitarybuilding" && $type != "getmilitarylocation" && $type != 'getallmilitarylocation' && $type != 'checkisbase' && $type != 'CheckUserOnBase' && $type != 'GetLiveTaskDetailDriverMultiRouting' && $type != "AllDriverRequests" && $type != "AllOrdertimeOrderWebOrdering" && $type != 'ChecllAllTesting' && $type != 'getpushnotifications' && $type != 'PlugPlayPayment' && $type != 'getlatestpoints' && $type != 'getTransactionHistory' && $type != 'DisplayUserAddress' && $type != "getBannerImages" && $type != "getExternalShops" && $type != 'getDetail' && $type != 'DisplayActiveOrder' && $type != 'GetOrderDetailsRestaurant') {
    // && $type != "SaveCustomerSrtipeid" && $type != "SavePostTipDetails"
...
$iServiceId = isset($_REQUEST["iServiceId"]) ? $_REQUEST["iServiceId"] : '0';
...
function getPassengerDetailInfo($passengerID, $cityName) {
    global $generalobj, $obj, $demo_site_msg, $PHOTO_UPLOAD_SERVICE_ENABLE, $parent_ufx_catid, $generalConfigArr, $tconfig, $vTimeZone, $vUserDeviceCountry, $connMsSql;

    $where = " iUserId = '" . $passengerID . "'";
    $appVersion = isset($_REQUEST['AppVersion']) ? trim($_REQUEST['AppVersion']) : '';
    //$data_version['iAppVersion'] = "2";
    if ($appVersion > 0) {
        $data_version['iAppVersion'] = round($appVersion, 2);
    ...
    if (count($row) > 0) {
        ...
        $sql = "SELECT * from [dbo].[LOYAL_CUST] where LOYAL_CUS_NUM='" . $row[0]['card_number'] . "' and  EMAIL_ADDR='" . $row[0]['vEmail'] . "'";
        $stmt = sqlsrv_query($connMsSql, $sql);
        if ($stmt === false) {
            $returnArr['Action'] = "0";
        ...
        if ($row1) {
            ...
            $cardnumber = (int) ltrim($row1['LOYAL_CUS_NUM'], 0);
			$lastdigit = generatecheckdigit("$cardnumber");
            $row[0]['card_number'] = $cardnumber . $lastdigit;
            $row[0]['card_number_orig'] = $cardnumber; //.'9';
            $row[0]['loyality_points'] = $row1['PNTS'];
            $row[0]['redeem_points_value'] = $row1['RDMPT_VAL'];

            $tblname = "register_user";
            $iMemberId = 'iUserId';
            $Data_passenger = array();
            $Data_passenger['loyality_points'] = $row1['PNTS'];
            $Data_passenger['redeem_points_value'] = $row1['RDMPT_VAL'];
            $where = " $iMemberId = '" . $passengerID . "'";
            $Update_Member_id = $obj->MySQLQueryPerform($tblname, $Data_passenger, 'update', $where);
        } else {
            if ($row[0]['card_number'] != '') {
                $cardnumber = (int) ltrim($row['LOYAL_CUS_NUM'], 0);
				$lastdigit = generatecheckdigit("$cardnumber");
                $row[0]['card_number'] = $cardnumber . $lastdigit;
                $row[0]['card_number_orig'] = $cardnumber; //.'9';
            } else {
                $cardnumber = (int) ltrim($row['LOYAL_CUS_NUM'], 0);
                $row[0]['card_number'] = $row['LOYAL_CUS_NUM'];
                $row[0]['card_number_orig'] = $row['LOYAL_CUS_NUM']; //.'9';
            }
            $row[0]['loyality_points'] = $row[0]['loyality_points'];
            $row[0]['redeem_points_value'] = $row[0]['redeem_points_value'];
        }
        //sqlsrv_close( $connMsSql );
        $row[0]['csv_otp'] = $row[0]['csv_otp'];
        $dBirthDate = $row[0]['dBirthDate'];
        if ($row[0]['dBirthDate'] != '') {
            $dBirthDate = date('d/m/Y', strtotime($row[0]['dBirthDate']));
        }
        $row[0]['dBirthDate'] = $dBirthDate;
        $row[0]['eGender'] = $row[0]['eGender'];
        ...
        $sql = "SELECT ord.iOrderId,ord.iCompanyId,ord.iDriverId,ord.posttipvalue,ord.eIsScheduled,ord.dScdeliveryTime,ord.dScdeliveryDate FROM `orders` as ord WHERE ord.iUserId='" . $passengerID . "' AND ord.iStatusCode = '6' ORDER BY ord.iOrderId DESC LIMIT 0,1";
        ...
        if (count($row_order) > 0) {
            ...
            $posttipvalue = $row_order[0]['posttipvalue'];
            ...
            $sqlc = "SELECT iCompanyId,vCompany AS CompanyName,vImage, vCoverImage FROM company WHERE iCompanyId = '" . $LastOrderCompanyId . "'";
            ...
            $vLastOrderCompanyImg = '';
            $vLastOrderCompanyCoverImg = '';
            if ($result_company[0]['vImage'] != "") {
                $vLastOrderCompanyImg = $tconfig['tsite_upload_images_compnay'] . '/' . $result_company[0]['iCompanyId'] . '/' . $result_company[0]['vImage'];
            }
            if ($result_company[0]['vCoverImage'] != "") {
                $vLastOrderCompanyCoverImg = $tconfig['tsite_upload_images_compnay'] . '/' . $result_company[0]['iCompanyId'] . '/' . $result_company[0]['vCoverImage'];
            }

            $row[0]['LastOrderId'] = $LastOrderId;
            $row[0]['posttipvalue'] = $posttipvalue;
            $row[0]['LastOrderCompanyId'] = $LastOrderCompanyId;
            $row[0]['LastOrderCompanyName'] = $result_company[0]['CompanyName'];
            $row[0]['vLastOrderCompanyImg'] = $vLastOrderCompanyImg;
            $row[0]['vLastOrderCompanyCoverImg'] = $vLastOrderCompanyCoverImg;
            $row[0]['LastOrderDriverId'] = $LastOrderDriverId;
            $row[0]['LastOrderDriverName'] = $result_driver[0]['driverName'];
            $row[0]['eIsScheduled'] = $row_order[0]['eIsScheduled'];
            $row[0]['dScdeliveryTime'] = $row_order[0]['dScdeliveryTime'];
            $row[0]['dScdeliveryDate'] = $row_order[0]['dScdeliveryDate'];
        } else {
        ...
        $row[0]['SelectedPaymentGateway'] = getSelectedPaymentGatewayValue();
        $row[0]['iProfileId'] = $row[0]['iProfileId'] ? $row[0]['iProfileId'] : "0";
        $row[0]['iAccountId'] = $row[0]['iAccountId'] ? $row[0]['iAccountId'] : "0";
...
function getDriverDetailInfo($driverId, $fromSignIN = 0) {
    ...
    if (count($Data) > 0) {
        ...
        $sql = "SELECT ord.iOrderId,ord.iCompanyId,ord.iUserId,ord.iUserAddressId,ord.fNetTotal,ord.vOrderNo, ord.posttipvalue FROM `orders` as ord WHERE ord.iDriverId='" . $driverId . "' AND ord.iStatusCode = '6' AND (select count(iRatingId) from ratings_user_driver as rud where rud.iOrderId=ord.iOrderId AND rud.eFromUserType = 'Driver' ) = 0  ORDER BY ord.iOrderId DESC LIMIT 0,1";
        $row_order = $obj->MySQLSelect($sql);
        ...
        if (count($row_order) > 0) {
            ...
            $Data[0]['posttipvalue'] = $posttipvalue;
        ...
        $Data[0]['SelectedPaymentGateway'] = getSelectedPaymentGatewayValue();
        return $Data[0];
    } else {
        $returnArr['Action'] = "0";
        $returnArr['eStatus'] = "";
...
if ($type == 'generalConfigData') {
    ...
    if (count($db_customnotif) > 0) {
        ...
        foreach ($db_customnotif as $key => $value) {
            $array_custompush[$i]['lat'] = $value['lat'];
            $array_custompush[$i]['lon'] = $value['lon'];
            $array_custompush[$i]['message'] = $value['message'];
            $i++;
        }
    }

    $DataArr["notification"] = $array_custompush;
...
if ($type == "signup") {
    ...
    $vLang = isset($_REQUEST["vLang"]) ? $_REQUEST["vLang"] : '';
    $fcm_token = isset($_REQUEST["fcm_token"]) ? $_REQUEST["fcm_token"] : '';
    $user_city = isset($_REQUEST["user_city"]) ? $_REQUEST["user_city"] : '';
    $eGender = isset($_REQUEST["eGender"]) ? $_REQUEST["eGender"] : '';
    $tDestinationAddress = isset($_REQUEST["tDestinationAddress"]) ? $_REQUEST["tDestinationAddress"] : '';
    ...
    $iAccountID = isset($_REQUEST["iAccountID"]) ? $_REQUEST["iAccountID"] : 0;
    $card_number = isset($_REQUEST["card_number"]) ? $_REQUEST["card_number"] : 0;
    $user_loyalty_card_number = isset($_REQUEST["user_loyalty_card_number"]) ? $_REQUEST["user_loyalty_card_number"] : 0;
    //$barcode = isset($_REQUEST["barcode"]) ? $_REQUEST["barcode"] : '';

    if ($email == "" && $phone_mobile == "") {
        $returnArr['Action'] = "0";
        $returnArr['message'] = "LBL_TRY_AGAIN_LATER_TXT";
        echo json_encode($returnArr);
        exit;
    ...
    if (strlen($phone_mobile) > 10) {
        $returnArr['Action'] = "0";
        $returnArr['message'] = "LBL_PHONE_VALID_DIGITD_MSG";
        echo json_encode($returnArr);
        exit;
    }
    if ($vCurrency == '') {
    ...
    /*
      if($barcode != ''){
      $sql = "SELECT * FROM $tblname WHERE 1=1 AND barcode = '$barcode'";
      $check_passenger = $obj->MySQLSelect($sql);
      if($email == ''){
      $returnArr['Action'] = "0";
      $returnArr['message'] = "LBL_TRY_AGAIN_LATER_TXT";
      ...
      if($check_passenger[0]['vEmail'] == ''){
      $result = updatexistingbarcodeuser($tblname,$iMemberId,$check_passenger[0]['iUserId'],$Fname,$Lname,$email,$phone_mobile,$Password_passenger,$iGcmRegId,$vFirebaseDeviceToken,$vLang,$CountryCode,$deviceType);
      $returnArr['message'] = getPassengerDetailInfo($check_passenger[0]['iUserId'],'');
      echo json_encode($returnArr);
      exit;
      }
      if(strtolower($email) != strtolower($check_passenger[0]['vEmail']) OR $phone_mobile != $check_passenger[0]['vPhone']){
      $returnArr['Action'] = "0";
    ...
    if (count($check_passenger) > 0) {
        ...
        $Data_passenger['eGender'] = $eGender;
        $Data_passenger['tDestinationAddress'] = $tDestinationAddress;
        ...
        $Data_passenger['user_city'] = $user_city;
        $Data_passenger['fcm_token'] = $fcm_token;
        $Data_passenger['loyalty_card_number'] = time();
        $Data_passenger['user_loyalty_card_number'] = $user_loyalty_card_number;
        $Data_passenger['eDeviceType'] = $deviceType;
        $Data_passenger['iAccountID'] = $iAccountID;
        ...
        //$Data_passenger['card_number'] = mt_rand(10000000, 99999999);
        ...
        if (!$row) {
            $LOYAL_CUS_NUM = getNewCardNumber($connMsSql);
            ...
            if ($stmt === false) {
                //echo '<pre>'; print_r($params);die( print_r( sqlsrv_errors(), true));
                $returnArr['Action'] = "0";
                $returnArr['message'] = "LBL_SERVER_COMM_ERROR";//BBD_ERR
                echo json_encode($returnArr);
                //sqlsrv_close( $connMsSql );
                exit;
            }
           // $Data_passenger['card_number'] = $LOYAL_CUS_NUM;
            ...
            //$Data_passenger['card_number'] = $row['LOYAL_CUS_NUM'];
            $Data_passenger['loyality_points'] = $row['PNTS'];
            $Data_passenger['redeem_points_value'] = $row['RDMPT_VAL'];
        }
        //echo '<pre>';print_r($params);print_r($Data_passenger);die;
...
function getNewCardNumber($connMsSql) {
    global $obj;
    $pad_length = 20;
    $pad_char = 0;
    ...
    $stmt1 = sqlsrv_query($connMsSql, $sql);
    if ($stmt1 === false) {
        $returnArr['Action'] = "0";
        $returnArr['message'] = "LBL_SERVER_COMM_ERROR";
        echo json_encode($returnArr);
    ...
    if ($row1) { 
        if ($cardnumberid != '') {
            ...
            $data = array('card_number' => $GeneratedCardnumber, 'date_created'=>date('Y-m-d H:i:s'));
            $Update_card = $obj->MySQLQueryPerform('card_number', $data, 'update', $where);
        }
        getNewCardNumber($connMsSql);
...
if ($type == "update_additional_information") {
    ...
    $sql = "SELECT iUserId,eStatus,vEmail FROM `" . $tblname . "` WHERE iUserId='$GeneralMemberId'";
    $Data = $obj->MySQLSelect($sql); //[EMAIL_ADDR]
    //print_r($Data);
    $card_number1 = 0;
    $PNTS = 0;
    $RDMPT_VAL = 0;
    if (!empty($card_number)) {
        $card_number1 = str_pad($card_number, $pad_length, $pad_char, STR_PAD_LEFT);


        $sql = "SELECT * from [dbo].[LOYAL_CUST] where LOYAL_CUS_NUM='" . $card_number1 . "'";
        $stmt1 = sqlsrv_query($connMsSql, $sql);
        if ($stmt1 === false) {
            $returnArr['Action'] = "0";
            $returnArr['message'] = "LBL_SERVER_COMM_ERROR";
        ...
        $invalidcred = false;
        if ($row1) {
            $sql = "SELECT * from [dbo].[LOYAL_CUST] where LOYAL_CUS_NUM='" . $card_number1 . "' and  EMAIL_ADDR='" . $Data[0]['vEmail'] . "'";
            $stmt = sqlsrv_query($connMsSql, $sql);
            if ($stmt === false) {
                $returnArr['Action'] = "0";
                $returnArr['message'] = "LBL_SERVER_COMM_ERROR";
            ...
            $row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC);
            if ($row) {
                if ($row['LOYAL_CUS_NUM'] == $card_number1) {
                    $carderror = true;
                } else {
                    $invalidcred = true;
                }
        ...
        //sqlsrv_close( $connMsSql );
        if ($carderror) {
            $Data_passenger['card_number'] = $card_number1;
            $Data_passenger['loyality_points'] = $row['PNTS'];
            $Data_passenger['redeem_points_value'] = $row['RDMPT_VAL'];
            if ($row['CUST_NAME'] != '') {
                $customername = explode(' ', $row['CUST_NAME']);
                $firstname = $lastname = '';
                $fi = 0;
                foreach ($customername as $val) {
                    if ($i == 0) {
                        $firstname = $val . ' ';
                    } else {
                        $lastname .= $val . ' ';
                ...
                $Data_passenger['vName'] = trim($firstname);
                $Data_passenger['vLastName'] = trim($lastname);
            }

            $PNTS = $row['PNTS'];
            $RDMPT_VAL = $row['RDMPT_VAL'];
        }
    }
    if ($eGender != '') {
        $Data_passenger['eGender'] = $eGender;
    ...
    if ($dBirthDate != '') {
        $Data_passenger['dBirthDate'] = date('Y-m-d', strtotime(str_replace('/', '-', $dBirthDate)));
    }
    if ($tDestinationAddress != '') {
        $Data_passenger['tDestinationAddress'] = $tDestinationAddress;
    }

    //echo $dBirthDate.'--'.str_replace('/', '-', $dBirthDate).'-----'.date('Y-m-d',strtotime(str_replace('/', '-', $dBirthDate)));

    $iMemberId = 'iUserId';
    $where = " $iMemberId = '" . $GeneralMemberId . "'";
    //print_r($Data_passenger);die;
    //echo $tblname.'--'.'update'.'--'.$where;die;
    $Update_Member_id = $obj->MySQLQueryPerform($tblname, $Data_passenger, 'update', $where);
    $returnArr['Action'] = "1";
    $cardnumber = (int) ltrim($card_number1, 0);
    if ($cardnumber != '') {
        $card_number1 = $cardnumber; //$cardnumber.'9';
    } else {
        $card_number1 = $cardnumber;
    }

    if (!empty($card_number) && $carderror) {
        $returnArr['message'] = array("card_number" => ltrim($card_number1, 0), 'loyality_points' => $PNTS, 'redeem_points_value' => $RDMPT_VAL, 'tDestinationAddress' => $tDestinationAddress, 'dBirthDate' => $dBirthDate, 'eGender' => $eGender, 'GeneralMemberId' => $GeneralMemberId, 'message' => "LBL_CARDNUMBER_VALID");
    } elseif (empty($card_number) && !$carderror) {
        $returnArr['message'] = array("card_number" => ltrim($card_number1, 0), 'loyality_points' => $PNTS, 'redeem_points_value' => $RDMPT_VAL, 'tDestinationAddress' => $tDestinationAddress, 'dBirthDate' => $dBirthDate, 'eGender' => $eGender, 'GeneralMemberId' => $GeneralMemberId, 'message' => "LBL_CARDNUMBER_INVALID");
    } elseif ($invalidcred) {
        $returnArr['Action'] = "0";
        $returnArr['message'] = "LBL_CREDENTIALS_INVALID";
    } else {
        $returnArr['Action'] = "0";
        $returnArr['message'] = "LBL_CARDNUMBER_INVALID";
    }
    echo json_encode($returnArr);
    exit;
}
...
function updatexistingbarcodeuser($tblname, $iMemberId, $id, $Fname, $Lname, $email, $phone_mobile, $Password_passenger, $iGcmRegId, $vFirebaseDeviceToken, $vLang, $CountryCode, $deviceType) {
    global $obj;
    $Data_passenger['vName'] = $Fname;
    $Data_passenger['vLastName'] = $Lname;
    $Data_passenger['vEmail'] = $email;
...
if ($type == "getlatestpoints") {
	...
	$Data_passenger = array();
	if($iUserId == ''){
		$Data_passenger['loyality_points'] = 0;
		$Data_passenger['redeem_points_value'] = 0;
		$returnArr['Action'] = "1";
		$returnArr['message'] = $Data_passenger;
		echo json_encode($returnArr);
		exit;
	}
	...
	if ($UserType == "Passenger") {
		...
		if (count($row) > 0) {			
			...
			if ($row1) {
				...
				$Data_passenger['loyality_points'] = $row[0]['loyality_points'];
				$Data_passenger['redeem_points_value'] = $row[0]['redeem_points_value'];
			}
		}
	}
	$returnArr['Action'] = "1";
    ...
    if ($UserType == "Passenger") {
        ...
        if (count($Data) > 0) {
            ...
            if (($vLangCode != $Data[0]['vLang']) || $Data[0]['eChangeLang'] == "Yes") {
                ...
                $StripeCustomeid = "SELECT vStripeCusId, iProfileId, iAccountId FROM  `register_user` WHERE iUserId = '" . $iUserId . "';";
                ...
                $returnArr['iProfileId'] = $StripeCustomeidVal[0]['iProfileId'] ? $StripeCustomeidVal[0]['iProfileId'] : 0;
                $returnArr['iAccountId'] = $StripeCustomeidVal[0]['iAccountId'] ? $StripeCustomeidVal[0]['iAccountId'] : 0;
            ...
            $returnArr['message']["notification"] = $array_custompush;
            //print_r($returnArr);
...
###########################################################################
if ($type == 'updateDefaultAddress') {
    $iUserId = $_REQUEST["iUserId"];
    $iUserAddressId = $_REQUEST["iUserAddressId"];
    
    $where = " iUserId='" . $iUserId . "'";
    $data['isDefault'] = 0;
    $obj->MySQLQueryPerform("user_address", $data, 'update', $where);

    $where = " iUserId='" . $iUserId . "' AND iUserAddressId='".$iUserAddressId."'";
    $data['isDefault'] = 1;
    $obj->MySQLQueryPerform("user_address", $data, 'update', $where);

    $returnArr["Action"] = "1";
    $returnArr["message"] = 'Updated';
    //$returnArr['eFlatTrip'] = $eFlatTrip;
    echo json_encode($returnArr);
}
###########################################################################
...
//     $dBirthDate = isset($_REQUEST["dBirthDate"]) ? $_REQUEST["dBirthDate"] : '';
//     $tDestinationAddress = isset($_REQUEST["tDestinationAddress"]) ? $_REQUEST["tDestinationAddress"] : '';
//     $tDestinationLatitude = isset($_REQUEST["tDestinationLatitude"]) ? $_REQUEST["tDestinationLatitude"] : '';
//     $tDestinationLongitude = isset($_REQUEST["tDestinationLongitude"]) ? $_REQUEST["tDestinationLongitude"] : '';
...
//     }
//     if ($dBirthDate != '') {
//         $Data_update_User['dBirthDate'] = date('Y-m-d', strtotime(str_replace('/', '-', $dBirthDate)));
//     }
//     if ($tDestinationAddress != '') {
//         $Data_update_User['tDestinationAddress'] = $tDestinationAddress;
//     }
//     if ($tDestinationLatitude != '') {
//         $Data_update_User['tDestinationLatitude'] = $tDestinationLatitude;
//     }
//     if ($tDestinationLongitude != '') {
//         $Data_update_User['tDestinationLongitude'] = $tDestinationLongitude;
//     }
//     $card_number1 = 0;
//     $PNTS = 0;
//     $RDMPT_VAL = 0;
...
//         }
//         if ($carderror) {
//             $Data_update_User['card_number'] = $card_number1;
//             $Data_update_User['loyality_points'] = $row['PNTS'];
//             $Data_update_User['redeem_points_value'] = $row['RDMPT_VAL'];
//             if ($row['CUST_NAME'] != '') {
//                 $customername = explode(' ', $row['CUST_NAME']);
//                 $firstname = $lastname = '';
//                 $fi = 0;
...
//                     $i++;
//                 }
//                 $Data_update_User['vName'] = trim($firstname);
//                 $Data_update_User['vLastName'] = trim($lastname);
//             }
...
if ($type == "updateUserProfileDetail") {
    ...
    if ($userType == "Passenger") {
        ...
        $Data_update_User['isDefault'] = $isDefault;
        $Data_update_User['vPhoneCode'] = $phoneCode;
        $Data_update_User['vCurrencyPassenger'] = $currencyCode;
        $currentLanguageCode = get_value('register_user', 'vLang', 'iUserId', $iMemberId, '', 'true');
...
if ($type == "CheckUserOnBase") {
    ...
    $longitude = strip_tags($_REQUEST['longitude']);
    $result = array();
    $allowed = checkrestrictedareaonlyy($latitude, $longitude);
    if ($allowed == 1) {
...
if ($type == "getmilitarylocation") {
    ...
    $building = strip_tags($_REQUEST['building']);
    if (isset($building) && !empty($building)) {
        $sql = "SELECT militarylocations.id, militarylocations.base, militarylocations.region, militarylocations.building, militarylocations.latitude, militarylocations.longitude from militarylocations left join baselocations on baselocations.vLocationName = militarylocations.base WHERE baselocations.eStatus = 'Active' AND militarylocations.eStatus = 'Active' AND base = '$basename'  AND building = '$building'; ";
    } else {
        $sql = "SELECT militarylocations.id, militarylocations.base, militarylocations.region, militarylocations.building, militarylocations.latitude, militarylocations.longitude from militarylocations left join baselocations on baselocations.vLocationName = militarylocations.base WHERE baselocations.eStatus = 'Active' AND militarylocations.eStatus = 'Active'";
    }
    $result = $obj->MySQLSelect($sql);
    $finalresponse = array();
    if (!empty($result)) {
...
//        }
    ...
    if ($alertSendAllowed == true) {
        ...
        if (count($deviceTokens_arr_ios) > 0) {
            $final_message['apns'] = array("payload" => array("aps" => array("mutable-content" => 1)));
            // sendApplePushNotification(1,$deviceTokens_arr_ios,$message,$alertMsg,1);
            sendApplePushNotification(1, $deviceTokens_arr_ios, $final_message, $alertMsg, 0);
        }
    }
...
if ($type == "cancelTrip") {
    ...
    if ($userType != "Driver") {
        $message = $Message = "TripCancelled";
    } else {
        $message = $Message = "TripCancelledByDriver";
    }
    ...
    if ($alertSendAllowed == true) {
        ...
        if (count($deviceTokens_arr_ios) > 0) {
            ...
            if ($userType == "Driver") {
                ...
                sendApplePushNotification(1, $deviceTokens_arr_ios, $message_arr, $alertMsg, 0);
            }
        }
    }
        ...
        if ($driverStatus == "Active") {
            ...
            if ($alertSendAllowed == true) {
                if ($data[0]['deviceType'] == "Android" /* && $ENABLE_PUBNUB != "Yes" */) {
                    ...
                    if ($message != "") {
                        $message_arr['message'] = 'DestinationAdded';
                        $message_arr['title'] = $alertMsg;
                        $message_arr['apns'] = array("payload" => array("aps" => array("mutable-content" => 1)));
                        if ($userType == "Driver") {
                            sendApplePushNotification(0, $deviceTokens_arr_ios, $message_arr, $alertMsg, 0);
...
if ($type == "submitRating") {
    ...
    if (count($row_check) > 0) {
        ...
        if ($eFromUserType == "Passenger") {
            ...
            $Data_update_ratings1['iTripId'] = 0;
...
//    }
    ...
    if ($iOrderId > 0) {
        ...
        $Data_update_driver['vAvailability'] = "Available";
        $id = $obj->MySQLQueryPerform("register_driver", $Data_update_driver, 'update', $where);
...
function acceptorderpushnotification($result_company, $iCompanyId, $iUserId, $result, $message, $alertMsg) {
    ...
    if ($ENABLE_PUBNUB == "Yes" && $PUBNUB_PUBLISH_KEY != "" && $PUBNUB_SUBSCRIBE_KEY != "") {
        ...
        pubnubnotification($channelName, $message_pub_company);
    ...
    if ($alertSendAllowed == true) {
        if ($result[0]['eDeviceType'] == "Android") {
            ...
            //$alertMsg = $tripdriverarrivlbl;
            array_push($deviceTokens_arr_ios, $result[0]['iGcmRegId']);
            $newmessage = json_decode($message);
            $newmessage['message'] = $newmessage['Message'];
            $newmessage['title'] = $alertMsg;
            $newmessage['apns'] = array("payload" => array("aps" => array("mutable-content" => 1)));
            sendApplePushNotification(0, $deviceTokens_arr_ios, $message_arr, $alertMsg, 0);
        }
        ...
        if ($result_company[0]['eDeviceType'] == "Android") {
            ...
            $newmessage['apns'] = array("payload" => array("aps" => array("mutable-content" => 1)));
            array_push($deviceTokens_arr_ios_company, $result_company[0]['iGcmRegId']);
            sendApplePushNotification(2, $deviceTokens_arr_ios_company, $newmessage, $alertMsg, 0);
        }
    }
}
    ...
    if ($id > 0) {
        ...
        if ($alertSendAllowed == true) {
            ...
            if ($result[0]['eDeviceType'] == "Android") {
                ...
                $message_arr['title'] = $alertMsg;
                $message_arr['apns'] = array("payload" => array("aps" => array("mutable-content" => 1)));
                sendApplePushNotification(0, $deviceTokens_arr, $message_arr, $alertMsg, 0);
            }
        }
    ...
    if ($id > 0) {
        ...
        if ($alertSendAllowed == true) {
            ...
            if ($eDeviceType == "Android") {
                ...
                $message_arr['message'] = $message_arr['Message'];
                $message_arr['title'] = $alertMsg;
                $message_arr['apns'] = array("payload" => array("aps" => array("mutable-content" => 1)));
                sendApplePushNotification(0, $deviceTokens_arr, $message_arr, $alertMsg, 0);
            }
...
if ($type == "GenerateCustomer") {
    ...
    if ($eMemberType == "Passenger") {
        ...
        $profileData = getDriverDetailInfo($iUserId, 0);
    }
...
if ($type == "getBanners") {
    ...
    $sql = "SELECT vImage FROM `banners` WHERE vCode = '" . $vLanguage . "' AND eStatus = 'Active' AND iServiceId = '" . $iServiceId . "' AND type = 'restaurants' ORDER BY iDisplayOrder ASC";
    ...
    for ($i = 0; $i < count($banners); $i++) {
        if ($banners[$i]['vImage'] != "") {
            $data[$count]['vImage'] = $tconfig["tsite_url"] . 'hurleys_backend/assets/img/images/' . $banners[$i]['vImage'];
            $count++;
        }
    }
    if (empty($data)) {
...
if ($type == "getHomeBanners") {
    ...
    //$banners= get_value('banners', 'vImage', 'vCode',$vLanguage,' ORDER BY iDisplayOrder ASC');
    $sql = "SELECT vImage FROM `banners` WHERE vCode = '" . $vLanguage . "' AND eStatus = 'Active' AND iServiceId = '" . $iServiceId . "' AND type = 'home' ORDER BY iDisplayOrder ASC";
    $banners = $obj->MySQLSelect($sql);
...
if ($type == "getCardDetails") {
    $iUserId = isset($_REQUEST['iUserId']) ? clean($_REQUEST['iUserId']) : '';

    //$banners= get_value('banners', 'vImage', 'vCode',$vLanguage,' ORDER BY iDisplayOrder ASC');
    $sql = "SELECT card_number, loyality_points, redeem_points_value FROM `register_user` WHERE iUserId = '" . $iUserId . "' ";
    $card_details = $obj->MySQLSelect($sql);

    $returnArr['Action'] = "1";
    $returnArr['message'] = $card_details;
    echo json_encode($returnArr);
}
...
if ($type == 'requestResetPassword') {
    ...
    if ($userType == "Passenger") {
        ...
        $tblname = "company";
        $fields = 'iCompanyId  as iMemberId, vPhone,vCode as vPhoneCode, vEmail, vCompany as vName, vPassword, vLang';
        $condfield = 'iCompanyId';
    ...
    $sql = "select $fields from $tblname where vEmail = '" . $Emid . "' and eStatus != 'Deleted'";
    $db_member = $obj->MySQLSelect($sql);
    if (count($db_member) > 0) {
        ...
        $maildata['OTP'] = $newToken;
        $status = $generalobj->send_email_user("CUSTOMER_RESET_PASSWORD_OTP", $maildata);
        if ($status == 1) {
            $sql = "UPDATE $tblname set vPassword_token='" . $newToken . "', vPassword_token_date='" . $vPassword_token_date . "' WHERE vEmail='" . $Emid . "' and eStatus != 'Deleted'";
            $obj->sql_query($sql);
...
if ($type == 'ResetPassword') {
    ...
    $userType = isset($_REQUEST["userType"]) ? $_REQUEST["userType"] : '';
    if (trim($otp) == '' || trim($newpassword) == '' || trim($vPassword) == '' || trim($userType) == '') {
        $returnArr['Action'] = "0";
        $returnArr['message'] = "Required parameters are missing.";
        $obj->MySQLClose();
        echo json_encode($returnArr);
        exit;
    }
    ...
    $sql = "select $fields from $tblname where vPassword_token = '" . $otp . "'";
    $db_member = $obj->MySQLSelect($sql);
    if (count($db_member) > 0) {
        if ($newpassword == $vPassword) {
            $sql = "UPDATE " . $tblname . " set vPassword='" . $generalobj->encrypt_bycrypt($vPassword) . "',vPassword_token='' WHERE " . $condfield . "='" . $db_member[0]["iMemberId"] . "'";
            $obj->sql_query($sql);
            $returnArr['Action'] = "1";
            $returnArr['message'] = "LBL_PASS_UPDATE_MSG";
        } else {
            $returnArr['Action'] = "0";
            $returnArr['message'] = "LBL_PASS_NOT_MATCH";
        }
    } else {
        $returnArr['Action'] = "0";
        $returnArr['message'] = "LBL_WRONG_OTP_TXT";
    }
    $obj->MySQLClose();
    echo json_encode($returnArr);
    exit;
...
/* For verifying OTP reset password */
if ($type == 'verify_otp_reset_password') {
    global $generalobj, $obj, $tconfig;
    $otp = isset($_REQUEST["otp"]) ? $_REQUEST["otp"] : '';
    //$newpassword = isset($_REQUEST["newpassword"]) ? $_REQUEST["newpassword"] : '';
    //$vPassword = isset($_REQUEST["vPassword"]) ? $_REQUEST["vPassword"] : '';
    $userType = isset($_REQUEST["userType"]) ? $_REQUEST["userType"] : '';
    if (trim($otp) == '' || trim($userType) == '') {
        $returnArr['Action'] = "0";
        $returnArr['message'] = "Required parameters are missing.";
        $obj->MySQLClose();
        echo json_encode($returnArr);
    ...
    if ($userType == "Passenger") {
        $tblname = "register_user";
        $fields = 'iUserId as iMemberId, vPhone,vPhoneCode as vPhoneCode, vEmail, vName, vLastName, vPassword, vLang, vPassword_token_date';
        $condfield = 'iUserId';
        $EncMembertype = base64_encode(base64_encode('rider'));
    } else if ($userType == "Company") {
        $tblname = "company";
    ...
    $db_member = $obj->MySQLSelect($sql);
    if (count($db_member) > 0) {
        $dbtimestamp = strtotime($db_member[0]['vPassword_token_date']);
        if (time() - $dbtimestamp > 1 * 60) {
            $returnArr['Action'] = "2";
            $returnArr['message'] = "LBL_CURRECT_OTP_EXPIRED_TXT";
        } else {
            $returnArr['Action'] = "1";
            $returnArr['message'] = "LBL_CURRECT_OTP_TXT";
        }
    } else {
        $returnArr['Action'] = "0";
        $returnArr['message'] = "LBL_WRONG_OTP_TXT";
...
/* For verifying Forgot Password */



/* For reset password */
if ($type == 'reset_password_after_verification') {
    global $generalobj, $obj, $tconfig;
    $otp = isset($_REQUEST["otp"]) ? $_REQUEST["otp"] : '';
    $newpassword = isset($_REQUEST["newpassword"]) ? $_REQUEST["newpassword"] : '';
    $vPassword = isset($_REQUEST["vPassword"]) ? $_REQUEST["vPassword"] : '';
...
if ($type == "DisplayUserAddressNew") {
    ...
    $vRestuarantLocationLat = $db_companydata[0]['vRestuarantLocationLat'];
    $vRestuarantLocationLong = $db_companydata[0]['vRestuarantLocationLong'];
    $user =  $obj->MySQLSelect("select * from register_user where iUserId = '".$iUserId."'");
    $data['user'] = $user;
    
    $address =  $obj->MySQLSelect("select * from user_address where iUserId= '".$iUserId."'");
    $data['address'] = $address;
    $returnArr['Action'] = "1";
     $returnArr['message'] = $data;
    $obj->MySQLClose();
    echo json_encode($returnArr);
    // $db_userdata = $obj->MySQLSelect($user, $address);
    // $db_userdata_new = array();
    // $db_userdata_new = $db_userdata;
    // if (count($db_userdata) > 0) {
    //     for ($i = 0; $i < count($db_userdata); $i++) {
...
if ($type == "getExternalShops") {
    $sql = "select * from `external_shops` ORDER BY id DESC";
    $db_shopdata = $obj->MySQLSelect($sql);
    $shopdata = $db_shopdata;
    $i = 0;
    Foreach($db_shopdata as $db_shopdata_each){
        $shopdata[$i]['image'] = 'http://52.3.107.59/hurleys_backend/admin/images/external-shops/'.$db_shopdata_each['image'];
        $i++;
    }
    $returnArr['Action'] = "1";
    $returnArr['message'] = $shopdata;
    $obj->MySQLClose();
    echo json_encode($returnArr);
    exit;
}
 ...
 * Get Banner Images.
 */
if ($type == "getBannerImages") {
    $sql = "select * from `banner_images` ORDER BY id ASC";
    $db_shopdata = $obj->MySQLSelect($sql);
    $db_banners['top_banners'][0]['image'] = 'http://52.3.107.59/hurleys_backend/admin/banners/'.$db_shopdata[0]['image'];
    $db_banners['top_banners'][0]['url'] = $db_shopdata[0]['url'];
    $db_banners['top_banners'][1]['image'] = 'http://52.3.107.59/hurleys_backend/admin/banners/'.$db_shopdata[1]['image'];
    $db_banners['top_banners'][1]['url'] = $db_shopdata[1]['url'];
    $db_banners['top_banners'][2]['image'] = 'http://52.3.107.59/hurleys_backend/admin/banners/'.$db_shopdata[2]['image'];
    $db_banners['top_banners'][2]['url'] = $db_shopdata[2]['url'];

    $db_banners['middle_banners'][0]['image'] = 'http://52.3.107.59/hurleys_backend/admin/banners/'.$db_shopdata[3]['image'];
    $db_banners['middle_banners'][0]['url'] = $db_shopdata[3]['url'];
    $db_banners['middle_banners'][1]['image'] = 'http://52.3.107.59/hurleys_backend/admin/banners/'.$db_shopdata[4]['image'];
    $db_banners['middle_banners'][1]['url'] = $db_shopdata[4]['url'];

    $db_banners['bottom_banners'][0]['image'] = 'http://52.3.107.59/hurleys_backend/admin/banners/'.$db_shopdata[5]['image'];
    $db_banners['bottom_banners'][0]['url'] = $db_shopdata[5]['url'];

    $returnArr['Action'] = "1";
    $returnArr['message'] = $db_banners;
    $obj->MySQLClose();
    echo json_encode($returnArr);
    exit;
}
...
if($type == "getVirtualCard"){

	$Data_Update['virtual_cards'] = '';
	$id = $obj->MySQLQueryPerform("loyality_cards", $Data_Update, 'insert');

	$virtual_cards = '4579'.sprintf('%07d',$id);
	$Data_Update['virtual_cards'] = $virtual_cards;
 	$where = " id = '" . $id . "'";
    $obj->MySQLQueryPerform("loyality_cards", $Data_Update, 'update', $where);
	
    $sql = "select * from loyality_cards where id='" . $id . "'";
    $card_details = $obj->MySQLSelect($sql);

    $returnArr['message'] = "Success";
	$returnArr['data'] = $card_details;
...
//Update virtual cards status



if($type == "updateVirtualCard"){
...
if ($type == "UpdateUserAddressDetails") {
    ...
    $vTimeZone = isset($_REQUEST["vTimeZone"]) ? $_REQUEST["vTimeZone"] : '';
    $address = isset($_REQUEST["address"]) ? $_REQUEST["address"] : '';
    $addresstype = isset($_REQUEST["address_type"]) ? $_REQUEST["address_type"] : '';
    ...
    $Data_User_Address['address'] = $address;
    $Data_User_Address['address_type'] = $addresstype;
    //echo $action;
    if ($action == "Add") { //echo 1; exit;
    ...
    /*if ($insertid > 0) {
        ...
        $returnArr['Action'] = "0";
        $returnArr['message'] = "LBL_TRY_AGAIN_LATER_TXT2";
    }*/
    $returnArr['Action'] = "1";
        $returnArr['AddressId'] = $insertid;
        $returnArr['message1'] = "LBL_ADDRSS_ADD_SUCCESS";
...
if ($type == "submitTripHelpDetail") {
    ...
    if ($id > 0) {
        ...
        $generalobj->send_email_user("TRIP_HELP_DETAIL", $maildata);
        $returnArr['Action'] = "1";
...
if ($type == "loadAvailableRestaurants") {
    ...
    $checkrestrictaddress = 1;
    $getAdressarray = array($passengerLat, $passengerLon);
    /* $checkuseronbase = CheckLocationOfBase($getAdressarray);
      $baselocation = '0';
      // print_r($checkuseronbase);
      if ($checkuseronbase['isbase'] != 'no') {
      $base = $checkuseronbase['base'];
      $baselocation = $checkuseronbase['id'];
      } */
    //echo $baselocation; exit;
    //if ($checkrestrictaddress == 1) {
    //echo 1;
    //getNearRestaurantArr($passengerLat, $passengerLon, $iUserId, $fOfferType, $searchword, $vAddress, $iServiceId);
    //echo 2;
    ...
    foreach ($Data as $k => $v) {
        ...
        $Data_name['listingOrder'][$k] = $v['listingOrder'];
    }
    //array_multisort($Data_name['listingOrder'], SORT_DESC, $Data_name[$sortfield], $sortorder, $Data);
    ...
    $Data_F = array_slice($Data_new, $start_limit, $per_page);
    ### Checking For Pagination ###
    //New code to make wingzone at first
    // echo microtime(); echo "<br/>";
    $WingZindex = 0;
    for ($k = 0; $k < count($Data_F); $k++) {
        if ($Data_F[$k]['vCompany'] == 'Wing Zone') {
            $WingZindex = $k;
        }
    }

    if ($WingZindex != 0) {
        $Data = array(0 => $Data_F[$WingZindex]);
        unset($Data_F[$WingZindex]);
        $Data = array_merge($Data, $Data_F);
    } else {
        $Data = $Data_F;
    }

    //New code close to make wingzone at first
...
if ($type == "GetOrderDetailsRestaurant") {
    ...
    if (!empty($db_orders)) {
        ...
        $vLangCode = $db_orders[0]['vLang'];
        if ($vLangCode == "" || $vLangCode == NULL) {
            $vLangCode = get_value('language_master', 'vCode', 'eDefault', 'Yes', '', 'true');
        }
        $languageLabelsArr = getLanguageLabelsArr($vLangCode, "1", $iServiceId);

        if ($db_orders[0]['iStatusCode'] == 2) {
            $returnArr['message']['nextcode'] = 5;
            $returnArr['message']['nextlabel'] = $languageLabelsArr['LBL_PREPARING_ORDER'];
        } else if ($db_orders[0]['iStatusCode'] == 5) {
            $returnArr['message']['nextcode'] = 6;
            $returnArr['message']['nextlabel'] = $languageLabelsArr['LBL_SUCCESS_FINISHED'];
        }
...
if ($type == "DeclineOrder") {
    ...
    if ($ENABLE_PUBNUB == "Yes" && $PUBNUB_PUBLISH_KEY != "" && $PUBNUB_SUBSCRIBE_KEY != "") {
        ...
        pubnubnotification($channelName, $message);
        if ($eDeviceType != "Android") {
            array_push($deviceTokens_arr_ios, $iGcmRegId);
        }
    }
...
if ($type == "ConfirmOrderByRestaurant") {
    ...
    $message_arr['iOrderId'] = $iOrderId;
    $message_arr['vOrderNo'] = $vOrderNo;
    $message_arr['nextcode'] = 5;
    $message_arr['nextlabel'] = $languageLabelsArr['LBL_PREPARING_ORDER'];
    $message_arr['vTitle'] = $alertMsg;
    $message_arr['tSessionId'] = $data_order[0]['tSessionId'];
...
######################### Change status of Order By Restaurant ############################################################
if ($type == "ChangeOrderStatusByRestaurant") {
    $iCompanyId = isset($_REQUEST["iCompanyId"]) ? $_REQUEST["iCompanyId"] : "";
    $iOrderId = isset($_REQUEST["iOrderId"]) ? $_REQUEST["iOrderId"] : "";
    $ron = isset($_REQUEST["ron"]) ? $_REQUEST["ron"] : "";
    $iStatusCode = isset($_REQUEST["iStatusCode"]) ? $_REQUEST["iStatusCode"] : "";

    $where = " iOrderId = '" . $iOrderId . "'";
    $Data_update_order['iStatusCode'] = $iStatusCode;
    $Data_update_order['ron'] = $ron;
    $Order_Update_Id = $obj->MySQLQueryPerform("orders", $Data_update_order, 'update', $where);
    ...
    $nextcode = 6;
    ## Send Notification To User ##
    $Message = "OrderPreparedByRestaurant";
    $sql = "select ru.iUserId,ru.iGcmRegId,ru.eDeviceType,ru.tSessionId,ru.iAppVersion,ru.vLang,ord.vOrderNo from orders as ord LEFT JOIN register_user as ru ON ord.iUserId=ru.iUserId where ord.iOrderId = '" . $iOrderId . "'";
    $data_order = $obj->MySQLSelect($sql);
    $vLangCode = $data_order[0]['vLang'];
    $vOrderNo = $data_order[0]['vOrderNo'];
    ...
    $message_arr['tSessionId'] = $data_order[0]['tSessionId'];
    $message_arr['title'] = "Order Prepared By Restaurant";
    $message = json_encode($message_arr, JSON_UNESCAPED_UNICODE);
    if ($PUBNUB_DISABLED == "Yes") {
        $ENABLE_PUBNUB = "No";
    }
...
######################### End Change status of Order By Restaurant #############################################
// Driver app Types
...
if ($type == "GetLiveTaskDetailDriverMultiRouting") {
    ...
    $sql = "SELECT iUserId,iDriverId,iCompanyId,iOrderId,vOrderNo,iUserAddressId,iStatusCode,ePaid,ePaymentOption,cookingtime,deliverytime,vInstruction,ron FROM orders where iStatusCode IN (3,4,5) AND iDriverId = '" . $iDriverId . "'";
    $returnArrData = $obj->MySQLSelect($sql);
    //echo "<pre>"; print_r($returnArrDatas); echo "</pre>";
...
// echo "<pre>"; print_r($TripsData); echo "</pre>";
            ...
            if (!empty($CompanyData)) {
                ...
                $returnArrDataNewPickup[$i]['ron'] = $returnArrData[$i]['ron'];
                ...
                $returnArrDataNewDelivery[$i]['ron'] = $returnArrData[$i]['ron'];
...
if ($type == "CheckOutOrderDetails") {
    ...
    $dScdeliveryDate = isset($_REQUEST["dScdeliveryDate"]) ? $_REQUEST["dScdeliveryDate"] : 'Today';
    $dScdeliveryTime = isset($_REQUEST["dScdeliveryTime"]) ? $_REQUEST["dScdeliveryTime"] : 'Now';
...
//    }
    ...
    $Data_insert['vOrderNo'] = CCordernumber();
	...
	if($dScdeliveryDate != 'Today'){
		$Data_insert['eIsScheduled'] = '1';
		$Data_insert['dScdeliveryDate'] = date("Y-m-d",strtotime(str_replace('-','/',$dScdeliveryDate)));
	}
	if($dScdeliveryTime != 'Now'){
		$Data_insert['dScdeliveryTime'] = $dScdeliveryTime;
	}
        ...
        if ($ePaymentOption == "Cash") {
            ...
            if ($alertSendAllowed == true) {
                if ($eDeviceType == "Android") {
                    ...
                    $message_arr['title'] = $alertMsg;
                    $message_arr['apns'] = array("payload" => array("aps" => array("mutable-content" => 1)));
                    //$Rmessage         = array("message" => $CompanyMessage,"title" => $alertMsg,'tSessionId'=>$tSessionId,'MsgCode'=>strval(time() . mt_rand(1000, 9999)),'iOrderId'=>$iOrderId,"apns"=>array("payload"=>array("aps"=>array("mutable-content"=>1))));
                    array_push($deviceTokens_arr_ios, $iGcmRegId);
                    sendApplePushNotification(2, $deviceTokens_arr_ios, $message_arr, $alertMsg, 0);
                }
...
if ($type == "CaptureCardPaymentOrder") {
    ...
    if ($ePaymentOption == "Card") {
        ...
        try {
            ...
            if ($fNetTotal == 0 || ($result['status'] == "succeeded" && $result['paid'] == "1")) {
                ...
                $data['ePaid'] = "Yes";
                $data['gatewayused'] = "Stripe";
                $data['iStatusCode'] = 1;
                $data['fNetTotal'] = $fNetTotal;
                //new code for time
                $data['tOrderRequestDate'] = date("Y-m-d H:i:s");
                //new code for time close
    ...
    if ($alertSendAllowed == true) {
        if ($eDeviceType == "Android") {
            ...
            $Rmessage = array("message" => $CompanyMessage, "title" => $alertMsg, 'tSessionId' => $tSessionId, 'MsgCode' => strval(time() . mt_rand(1000, 9999)), 'iOrderId' => $iOrderId, "apns" => array("payload" => array("aps" => array("mutable-content" => 1))));
            array_push($deviceTokens_arr_ios, $iGcmRegId);
            sendApplePushNotification(2, $deviceTokens_arr_ios, $Rmessage, $alertMsg, 0);
        }
...
if ($type == "CheckOutOrderEstimateDetails") {     //echo "<pre>";print_r($_REQUEST);exit;
    ...
    $sql = "select vCompany,fMaxOfferAmt,fTargetAmt,fOfferType,fOfferAppyType,iMaxItemQty,fOfferAmt,vFromMonFriTimeSlot1, vToMonFriTimeSlot1 from `company` where iCompanyId = '" . $iCompanyId . "'";
    $db_companydata = $obj->MySQLSelect($sql);
    $vCompany = $db_companydata[0]['vCompany'];
    $fMaxOfferAmt = $db_companydata[0]['fMaxOfferAmt'];
	...
	if($iCompanyId == 46){
		$period = new DatePeriod(
			new DateTime('+1 Day'), // Start date of the period
			new DateInterval('P1D'), // Define the intervals as Periods of 1 Day
			2 // Apply the interval 6 times on top of the starting date
		);
	}else{
		$period = new DatePeriod(
	...
	if(count($period) > 0) {
		foreach ($period as $day) {
			$fromdatetime = $day->format('Y-m-d').' '.$db_companydata[0]['vFromMonFriTimeSlot1'];
			$todatetime = $day->format('Y-m-d').' '.$db_companydata[0]['vToMonFriTimeSlot1'];
			$timeslots[$day->format('m-d-Y')] = SplitTime($fromdatetime,$todatetime);
		}
	}else{
		$fromdatetime 	= date('Y-m-d').' '.$db_companydata[0]['vFromMonFriTimeSlot1'];
		$todatetime 	= date('Y-m-d').' '.$db_companydata[0]['vToMonFriTimeSlot1'];
		$timeslots[date('m-d-Y')] = SplitTime($fromdatetime,$todatetime);
	}
	//print_r($days);
    ...
    if (!empty($OrderDetails)) {
        ...
        $fDeliveryCharge = getLoyalityAppdeliverycharge();
        $fTax = 0; //round(((($fFinalTotal + $fDeliveryCharge) * 7 ) / 100), 2);
        $preTipCharge = $preTip = 0;
...
function SplitTime($StartTime, $EndTime, $Duration="60"){
    ...
    while ($StartTime <= $EndTime) //Run loop
    {
		...
		$ReturnArray[] = date ("h:i a", $fromtime).' - '.date ("h:i a", $StartTime);
    }
    return $ReturnArray;
}
############################# Calculate Order Estimate Amount ###################################################################
...
if ($type == "getOrderDeliveryLog") {
    ...
    $sql = "SELECT os.vStatus,os.vStatus_Track,osl.dDate,osl.iStatusCode,ord.iUserId,ord.iCompanyId,ord.iDriverId,ord.cookingtime,ord.deliverytime,ord.iStatusCode as OrderCurrentStatusCode,ord.iUserAddressId,ord.vOrderNo,ord.tOrderRequestDate,ord.fNetTotal FROM order_status_logs as osl LEFT JOIN order_status as os ON osl.iStatusCode = os.iStatusCode LEFT JOIN orders as ord ON osl.iOrderId=ord.iOrderId WHERE osl.iOrderId = '" . $iOrderId . "' AND osl.iStatusCode NOT IN(" . $NotInStatusCode . ") ORDER BY osl.iOrderLogId ASC";
    ...
    if (count($OrderStatus) > 0) {
        ...
        $etamessage = array(
            '0' => '',
            '1' => 'Awaiting restaurant confirmation.',
            '2' => 'Restaurant is preparing your order.',
            '4' => $drivername . ' is enroute to pick up your order.',
            '5' => $drivername . ' is on the way.',
        ...
        for ($i = 0; $i < count($OrderStatus); $i++) {
            ...
            $OrderStatus[$i]['etamessage'] = $etamessage[$OrderStatus[$i]['iStatusCode']];
        }

        if ($CheckOtherStatusCode == "Yes" && $UserType == "Passenger") {
            $OrderStatusCode = substr($OrderStatusCode, 0, -1);
            $OrderStatusCode = $OrderStatusCode . ",4,7,8,9,11,12";
            ...
            for ($i = 0; $i < count($OrderStatusNotExist); $i++) {
                ...
                $OrderStatusNotExist[$i]['etamessage'] = $etamessage[$OrderStatusNotExist[$i]['iStatusCode']];
        ...
        $tOrderRequestDate = converToTz($tOrderRequestDate, $vTimeZone, $serverTimeZone);
        $pickupRequesttime = converToTz($OrderStatus[0]['cookingtime'], $vTimeZone, $serverTimeZone);
        $MaxDeliverytime = converToTz($OrderStatus[0]['deliverytime'], $vTimeZone, $serverTimeZone);
        $calculatedTimeFromgoogle = GetDrivingDistance($lat1, $lat2, $long1, $long2);
        ...
        $returnArr['etaPick'] = $pickupRequesttime;
        //$returnArr['etaPrePick'] = GetTimeDiffInMinutes($OrderStatus[0]['cookingtime']);
        $returnArr['etaDeliver'] = $MaxDeliverytime;
        //$returnArr['etaPostPick'] = GetTimeDiffInMinutes($OrderStatus[0]['deliverytime']);
        ...
        if ($OrderStatus[0]['iDriverId'] > 0) {
            ...
            $returnArr['DriverPhone'] = $Data_cab_driverlatlong[0]['vPhone'];
        } else {
            $returnArr['DriverLat'] = "";
            ...
            for ($j = 0; $j < count($Data); $j++) {
                ...
                if ($dateName == $dateName_tmp) {
                    ...
                    if ($Data[$j]['iStatusCode'] == '11' || $Data[$j]['iStatusCode'] == '9') {
                        ...
                        $status = $languageLabelsArr["LBL_HISTORY_REST_COMPLETED"];
                    } else {
                        $status = '';
                    }
...
//                        }
                        //New code for Earning Driver
                        $tipValue = 0;
                        $totalEarningDriver = 0;
                        //pretip postip and bonus
                        $tipValue = $Data[$j]['fpretip'] + $Data[$j]['posttip'];
                        if ($tipValue > 0) {
                        ...
                        //Reimbursement 
                        $reimbursement = calculateDriverReimbursement($Data[$j]['iOrderId']);
                        if ($reimbursement > 0) {
                            $totalEarningDriver = $totalEarningDriver + $reimbursement;
                        }

                        //bONUS
                        $RatingBonus = calculateDriverRatingBonus($Data[$j]['iOrderId']);
                        if ($RatingBonus > 0) {
                            $totalEarningDriver = $totalEarningDriver + $RatingBonus;
                        }

                        //New code for Earning Driver closed
                        //  $EarningFare = $Data[$j]['fDeliveryCharge'] + $Data[$j]['fpretip'] + $Data[$j]['posttip'];

                        $EarningFare = $totalEarningDriver;
                    } else if ($UserType == 'Passenger') {
        ...
        if ($UserType == 'Driver') {
            $DriverAllValues = OrderTotalEarningForLoyalityAppDriver($iGeneralUserId, $vConvertFromDate, $vConvertToDate, $UserType, $vTimeZone);
            $totalEarning = $DriverAllValues['totalEarning'];
            $returnData['TotalTip'] = $vSymbol . $DriverAllValues['totaltip'];
            $returnData['TotalReimbursement'] = $vSymbol . $DriverAllValues['totalreimbursement'];
            $returnData['TotalBonus'] = $vSymbol . $DriverAllValues['totalbonus'];
        } else if ($UserType == 'Passenger') {
...
if ($type == "UpdateOrderStatus") {
    $iCompanyId = isset($_REQUEST["iCompanyId"]) ? $_REQUEST["iCompanyId"] : "";
    $iOrderId = isset($_REQUEST["iOrderId"]) ? $_REQUEST["iOrderId"] : "";
    $iStatusCode = isset($_REQUEST["nextcode"]) ? $_REQUEST["nextcode"] : "";

    $where = " iOrderId = '" . $iOrderId . "'";
    $Data_update_order['iStatusCode'] = $iStatusCode;
    $Order_Update_Id = $obj->MySQLQueryPerform("orders", $Data_update_order, 'update', $where);

    $id = createOrderLog($iOrderId, $iStatusCode);
    ...
    if ($iStatusCode == 5) {
        ...
        $Message = "OrderPreparedByRestaurant";
        $message_arr['title'] = "Order Prepared By Restaurant";
        $message_arr['nextcode'] = 6;
        $message_arr['nextlabel'] = $languageLabelsArr['LBL_SUCCESS_FINISHED'];
    } else {
        $alertMsg = $languageLabelsArr['LBL_TRIP_FINISHED_TXT'];
        $Message = "OrderCompletedByRestaurant";
        $message_arr['title'] = "Order Completed By Restaurant";
        $message_arr['nextcode'] = 6;
        $message_arr['nextlabel'] = $languageLabelsArr['LBL_SUCCESS_FINISHED'];
    }
    $message_arr['Message'] = $Message;
    ...
    if ($Order_Update_Id > 0) {
        $returnArr['Action'] = "1";
        if ($iStatusCode == 5) {
            $returnArr['message'] = $languageLabelsArr["LBL_PREPARE_ORDER_BY_RESTAURANT_APP_TXT"];
        } else {
            $returnArr['message'] = $languageLabelsArr["LBL_TRIP_FINISHED_TXT"];
        }
        $generalobj->orderemaildata($iOrderId, 'Passenger');
    } else {
        $returnArr['Action'] = "0";
...
if ($type == "UpdateRestaurantAvailability") {
    ...
    if ($CALL_TYPE == "Display") {
        ...
        $Data_update_Companies['eLogout'] = 'No';
...
if ($type == "AllDriverRequests") {
    ...
    $GetDriverDetailsQuery = "Select vAvailability from register_driver where iDriverId = '$iDriverId';";
    $GetDriverDetails = $obj->MySQLSelect($GetDriverDetailsQuery);



//New Code for logs

    $eStatusDriverLog = $GetDriverDetails[0]['vAvailability'];
    /*
      $curr_date = date('Y-m-d H:i:s');

      $Newtime = strtotime($curr_date);

      $GetPrevdataq = "Select `logtime` from `driverslocationlogs` where `iDriverId` = '$iDriverId' ORDER BY `logtime` DESC limit 1";
      $GetPrevdata = $obj->MySQLSelect($GetPrevdataq);
      $oldtime = 0;
      if(count($GetPrevdata) > 0 )
      {
      $oldtime =  strtotime($GetPrevdata[0]['logtime']);
      }
      $Minutediff = ($Newtime - $oldtime);

      if($Minutediff > 120)
      {

      $sqlDriverLog = "INSERT INTO `driverslocationlogs` (`iDriverId`,`logtime`,`latitude`,`longitude`,`eStatus`) VALUES ('" . $iDriverId . "','" . $curr_date . "','" . $vLatitude . "','" . $vLongitude . "','" . $eStatusDriverLog . "')";
      $insert_DriverLog = $obj->sql_query($sqlDriverLog);
      }
     */
    /*
      try {
      //    $sqlDriverLog = "INSERT INTO `driverslocationlogs` (`iDriverId`,`logtime`,`latitude`,`longitude`,`eStatus`)
      //SELECT * FROM (SELECT '$iDriverId',CURRENT_TIMESTAMP() ,'$vLatitude','$vLongitude','$eStatusDriverLog') AS tmp
      //WHERE NOT EXISTS ( SELECT `iDriverLocLogId` FROM `driverslocationlogs` WHERE (CURRENT_TIMESTAMP() - `logtime`)  < 120 AND `iDriverId` = '$iDriverId'  ORDER BY `iDriverLocLogId` DESC LIMIT 1 )";
      $sqlDriverLog = "INSERT INTO `driverslocationlogs` (`iDriverId`,`logtime`,`latitude`,`longitude`,`eStatus`)
      SELECT * FROM (SELECT '$iDriverId',CURRENT_TIMESTAMP() ,'$vLatitude','$vLongitude','$eStatusDriverLog') AS tmp
      WHERE NOT EXISTS ( SELECT `iDriverLocLogId` FROM `driverslocationlogs` WHERE DATE_ADD(`logtime`, INTERVAL 2 MINUTE) > NOW() AND `iDriverId` = '$iDriverId'  ORDER BY `iDriverLocLogId` DESC LIMIT 1 )";

      $insert_DriverLog = $obj->sql_query($sqlDriverLog);
      }
      catch (Exception $e) {} */
//New code for logs closed

    $serverTimeZone = date_default_timezone_get();
    $tDate = converToTz($tDate, $serverTimeZone, $vTimeZone);
    $DriverrequestQ = "SELECT * FROM `driver_request` WHERE `iDriverId` = '$iDriverId' AND `tDate` > '$tDate' AND `eStatus` IN ('Timeout');";
    ...
    if (empty($Driverrequests)) {
        ...
        $returnArr['vAvailability'] = $GetDriverDetails[0]['vAvailability'];
    } else {
        ...
        foreach ($Driverrequests as $Driverrequest) {
            ...
            $iGcmRegId = $passengerData[0]['iGcmRegId'];
            $passengerFName = $passengerData[0]['vCompany'];
            if ($Driverrequest['type'] == 'assign') {
                $message_as = "CabRequested";
            } else {
                $message_as = "OrderCancelByAdmin";
            }
            $final_message[] = array(
                'querydate' => $tDate,
                'type' => $Driverrequest['type'],
                'Message' => $message_as,
...
if ($type == "SavePostTipDetails") {
    ...
    $posttipvalue = isset($_REQUEST["posttipvalue"]) ? $_REQUEST["posttipvalue"] : '';
    ...
    if (empty($orders)) {
        ...
        $data_update['posttipvalue'] = $posttipvalue;
...
if ($type == "CaptureCardPaymentOrderBYCustomerID") {
    ...
    $returnArr = array();
    if (!empty($posttipvalue) && !empty($iOrderId))
        if (($amount != 0) && ($posttipvalue == 'yes')) {
            $sql = "select * from orders where iOrderId='" . $iOrderId . "'";
            $data_order = $obj->MySQLSelect($sql);
            $iUserId = $data_order[0]['iUserId'];
            $fNetTotal = $data_order[0]['fNetTotal'];
            ...
            if ($ePaymentOption == "Card") {
                ...
                $currencyCode = $DefaultCurrencyData[0]['vName'];
                $currencyratio = $DefaultCurrencyData[0]['Ratio'];
                $price_new = $amount * $currencyratio;
                $price_new = round($price_new * 100, 2);
                $tDescription = "Amount charge for order no" . $vOrderNo;
                try {
                    if ($amount > 0) {
                    ...
                    if ($amount == 0 || ($result['status'] == "succeeded" && $result['paid'] == "1")) {
                        $where = " iOrderId = '$iOrderId'";
                        $data['ptTransactionId'] = ($amount == 0) ? "" : $result['id'];
                        $data['posttipvalue'] = $posttipvalue;
                        $data['posttip'] = $amount;
                        $id = $obj->MySQLQueryPerform("orders", $data, 'update', $where);
                        $OrderLogId = createOrderLog($iOrderId, "1");
                        $returnArr["Action"] = "1";
                ...
                $returnArr["Action"] = "0";
                $returnArr['message'] = "Invalid Payment Details";
            }
        } elseif ($amount == 0) {
            $where = " iOrderId = '$iOrderId'";
            $data['ptTransactionId'] = 0;
            $data['posttipvalue'] = 'skip';
            $data['posttip'] = $amount;
            $id = $obj->MySQLQueryPerform("orders", $data, 'update', $where);
            $returnArr["Action"] = "1";
            $returnArr['message'] = "Post Tip Skipped by User.";
        } else {
            $returnArr["Action"] = "0";
            $returnArr['message'] = "Invalid Data";
        }
    echo json_encode($returnArr);
    exit;
}
...
//if ($type == "getstripcustomers") {
//    $customer = Stripe_Customer::all(["limit" => 3]);
//    echo '<pre>';
//    print_r($customer);
//    die('adsf');
//}
...
if ($type == "getStripCustomerDetailFromOrderId") {
    $iOrderId = isset($_REQUEST["iOrderId"]) ? $_REQUEST["iOrderId"] : '';

    $GetOrderQ = "SELECT `orders`.`iOrderId`, `orders`.`iUserId`, `register_user`.`vStripeCusId`, `register_user`.`vEmail`,`register_user`.`vPhone` FROM `orders` JOIN `register_user` ON `register_user`.`iUserId` = `orders`.`iUserId` WHERE `orders`.`iOrderId` = '$iOrderId';";
    $orders = $obj->MySQLSelect($GetOrderQ);
    ...
    if (empty($orders)) {
        ...
        $result = array(
            'iOrderId' => $orders[0]['iOrderId'],
            'iUserId' => $orders[0]['iUserId'],
            'vStripeCusId' => $orders[0]['vStripeCusId'],
            'vEmail' => $orders[0]['vEmail'],
            'vPhone' => $orders[0]['vPhone'],
        );


        $returnArr['Action'] = "1";
        $returnArr['message'] = $result;
    }
    $obj->MySQLClose();
    echo json_encode($returnArr, JSON_UNESCAPED_UNICODE);
    exit;
...
############################## Capture Card Paymant of Order and Process With Customerid ####################################################################
if ($type == "CaptureCardPaymentOrderBYCustomerIDWithSignature") {
    $iOrderId = isset($_REQUEST["iOrderId"]) ? $_REQUEST["iOrderId"] : '';
    $ePaymentOption = isset($_REQUEST["ePaymentOption"]) ? $_REQUEST["ePaymentOption"] : '';
    $vStripeToken = isset($_REQUEST["vStripeToken"]) ? $_REQUEST["vStripeToken"] : '';
    $vStripeCusId = isset($_REQUEST["vStripeCusId"]) ? $_REQUEST["vStripeCusId"] : '';
    ...
    $PostTipExist = checkPostipExistfororder($iOrderId);
    if ($PostTipExist == 'Y') {
        $returnArr['Action'] = "0";
        $returnArr['message'] = "Tip for this order already exist.";
        echo json_encode($returnArr);
        exit;
    }
    ...
    if ($image_name != "") {
        $Photo_Gallery_folder = $tconfig['tsite_upload_order_images_path_sign'];
        if (!is_dir($Photo_Gallery_folder))
            mkdir($Photo_Gallery_folder, 0777);
        $vFile = $generalobj->fileupload($Photo_Gallery_folder, $image_object, $image_name, $prefix = '', $vaildExt = "bmp,pdf,doc,docx,jpg,jpeg,gif,png,xls,xlsx,csv");
        $vImageName = $vFile[0];
    ...
    if (!empty($posttipvalue) && !empty($iOrderId)) {
        if (($amount != 0) && ($posttipvalue == 'yes')) {
            ...
            if ($ePaymentOption == "Card") {
                ...
                $price_new = $amount * $currencyratio;
                $price_new = round($price_new * 100, 2);
                $tDescription = "Driver Tip for order no" . $vOrderNo;
                try {
                    if ($amount > 0) {
                        if ($vStripeCusId != '') {
                            $charge_create = Stripe_Charge::create(array(
                    ...
                    if ($amount == 0 || ($result['status'] == "succeeded" && $result['paid'] == "1")) {
                        ...
                        $sqlUserQ = "select ru.iUserId,ru.iGcmRegId,ru.eDeviceType,ru.tSessionId,ru.iAppVersion,ru.vLang,ord.vOrderNo from orders as ord LEFT JOIN register_user as ru ON ord.iUserId=ru.iUserId where ord.iOrderId = '" . $iOrderId . "'";
                        $data_user = $obj->MySQLSelect($sqlUserQ);
                        $alertMsg = "Thanks for the $" . $amount . " tip, enjoy your food!";
                        $Message = "OrderConfirmByDriver";
                        $message_arr = array();
                        $message_arr['Message'] = $Message;
                        $message_arr['iOrderId'] = $iOrderId;
                        $message_arr['vOrderNo'] = $data_user[0]['vOrderNo'];
                        $message_arr['vTitle'] = $alertMsg;
                        $message_arr['title'] = 'Delivery Completed';
                        $message_arr['tSessionId'] = $data_user[0]['tSessionId'];
                        ;
                        $message = json_encode($message_arr, JSON_UNESCAPED_UNICODE);
                        $alertSendAllowed = true;
                        $tableName = "register_user";
                        $iMemberId_VALUE = $iUserId;
                        $iMemberId_KEY = "iUserId";
                        $iAppVersion = $data_user[0]['iAppVersion'];
                        $eDeviceType = $data_user[0]['eDeviceType'];
                        $iGcmRegId = $data_user[0]['iGcmRegId'];
                        $tSessionId = $data_user[0]['tSessionId'];
                        $registatoin_ids = $iGcmRegId;
                        $deviceTokens_arr_ios = array();
                        $registation_ids_new = array();
            ...
            $sqlUserQ = "select ru.iUserId,ru.iGcmRegId,ru.eDeviceType,ru.tSessionId,ru.iAppVersion,ru.vLang,ord.vOrderNo from orders as ord LEFT JOIN register_user as ru ON ord.iUserId=ru.iUserId where ord.iOrderId = '" . $iOrderId . "'";
            $data_user = $obj->MySQLSelect($sqlUserQ);
            $alertMsg = "Thanks,  enjoy your food!";
            $Message = "OrderConfirmByDriver";
            $message_arr = array();
            $message_arr['Message'] = $Message;
            $message_arr['iOrderId'] = $iOrderId;
...
############################## Capture Card Paymant of Order ####################################################################
if ($type == "CaptureCardPaymentOrderWebOrdering") {
    $iOrderId = isset($_REQUEST["iOrderId"]) ? $_REQUEST["iOrderId"] : '';
    $ePaymentOption = isset($_REQUEST["ePaymentOption"]) ? $_REQUEST["ePaymentOption"] : '';
    $vStripeToken = isset($_REQUEST["vStripeToken"]) ? $_REQUEST["vStripeToken"] : '';
    $vStripeCusId = isset($_REQUEST["vStripeCusId"]) ? $_REQUEST["vStripeCusId"] : '';
...
//                    ));
//                    echo "No";
                    // New code to store customer ID
//                       try{
//                         $sqlUserQuery = "SELECT `vEmail` FROM `register_user` WHERE `iUserId` = '".$iUserId."';";
//                         $db_UserDetailsdata = $obj->MySQLSelect($sqlUserQuery);
//                        
//                         echo  $db_UserDetailsdata[0]['vEmail'];
//                        
//                        $customer = Stripe_Customer::create(array(
//                            'email' => $db_UserDetailsdata[0]['vEmail'],
//                            'source'  => $vStripeToken, 
//                        ));
//                        
//                        print_r($customer); 
//                                
//                        
//                        $StripeCustomerIds= $customer->id;
//                        
//                        $updateQury = "UPDATE `register_user` SET `vStripeCusId` = '".$StripeCustomerIds."' WHERE `register_user`.`iUserId` = '" . $iUserId . "';";
//                        $obj->sql_query($updateQury);
//                        
//                        exit;
//                        
...
############################## Capture Card Paymant of Order ####################################################################
############################## Phone Order Timing ####################################################################
if ($type == "AllOrdertimeOrderWebOrdering") {
...
############################## Phone Order Timing close ####################################################################
##############################Card Connect Apis WorkFlow Start ###################################

if ($type == "createCustomerProfileewqeqweqweqweq") {

    $Merchid = '800000000844';
    $iUserId = $_REQUEST['iUserId'] ? $_REQUEST['iUserId'] : '';
    $accttyppe = $_REQUEST['accttyppe'] ? $_REQUEST['accttyppe'] : 'VISA';
    $account = $_REQUEST['account'] ? $_REQUEST['account'] : '4444333322221111';
    $expiry = $_REQUEST['expiry'] ? $_REQUEST['expiry'] : '0924';
    $cvv2 = $_REQUEST['cvv2'] ? $_REQUEST['cvv2'] : '123';
    $amount = $_REQUEST['amount'] ? $_REQUEST['amount'] : '59';
    $iOrderId = $_REQUEST['iOrderid'] ? $_REQUEST['iOrderid'] : '123456';
    ...
    $request = array(
        ...
        'orderid' => $iOrderId,
        'name' => "Akash Singh",
        'address' => "132 rea street",
        'city' => "jacksonville",
        'region' => "North Calorina",
        'country' => "US",
        'postal' => "259687",
        'ecomind' => 'E',
        'tokenize' => "Y",
    ...
    $ch = curl_init('https://fts.cardconnect.com:6443/cardconnect/rest/auth');
    ...
    if ($responsep['resptext'] == 'Approval') {
        $where = " iOrderId = '" . $iOrderId . "'";
        $Data_update_order['cookingtime'] = $TimeAllocatedate['cookingtime'];
        $Data_update_order['pickuptime'] = $TimeAllocatedate['pickuptime'];
        $Data_update_order['mattime'] = $TimeAllocatedate['mattime'];
     ...
     * {
      "merchid": "800000000844",
      "accttype": "VISA",
      "orderid": "AB-13-0619001",
      "account": "4111111111111111",
      "expiry": "1223",
      "amount": "39",
      "currency": "USD",
      "name": "Akash Singh",
      "address": "123 REA STREET",
      "city": "jacksonville",
      "region": "NC",
      "country": "US",
      "postal": "28546",
    ...
    //{"amount":"0.39","resptext":"Approval","acctid":"1","cvvresp":"M","respcode":"000","batchid":"100","avsresp":"Z","defaultacct":"Y","merchid":"800000000844","token":"9418594164541111","authcode":"PPS803","respproc":"RPCT","profileid":"17564535816528114276","retref":"164800110222","respstat":"A","account":"41XXXXXXXXXX1111"}
    //{"amount":"39.00","resptext":"Approval","acctid":"1","cvvresp":"M","respcode":"000","batchid":"100","avsresp":"Z","defaultacct":"Y","merchid":"800000000844","token":"9418594164541111","authcode":"PPS815","respproc":"RPCT","profileid":"15075265802093085342","retref":"164824110713","respstat":"A","account":"41XXXXXXXXXX1111"}
    //  ProfileUpdates
    //{"country":"US","address":"123 REA STREET","resptext":"Profile Saved","city":"jacksonville","acctid":"1","respcode":"09","defaultacct":"Y","accttype":"VISA","token":"9441149619831111","license":"123451254","respproc":"PPS","phone":"7778789999","profileid":"17279693441593751006","name":"Akash Singh","auoptout":"N","postal":"28546","expiry":"0224","region":"NC","ssnl4":"3655","respstat":"A"}
}


if ($type == "GetCustomerProfile") {
    $iProfileId = $_REQUEST['iProfileId'] ? $_REQUEST['iProfileId'] : '';
    $iAccountId = isset($_REQUEST["iAccountId"]) ? $_REQUEST["iAccountId"] : '';
    //  $iUserId = isset($_REQUEST["iUserId"]) ? $_REQUEST["iUserId"] : '';
    //$userData = getRequiredCustomerDetailsFromId($iUserId);
    //print_r($userData); exit;
    $CCDetails = getCardConnectDetails();
    $Merchid = $CCDetails['CARDCONNECT_MERCHANTID'];
    $CCUrl = $CCDetails['CARDCONNECT_URL'] . 'profile';
    $CCUsername = $CCDetails['CARDCONNECT_USERNAME'];
    $CCpassword = $CCDetails['CARDCONNECT_PASSWORD'];
    if (!empty($iProfileId)) {
        $ch = curl_init("$CCUrl/$iProfileId/$iAccountId/$Merchid");
        ...
        $returnArr['Action'] = "0";
        $returnArr['message'] = "Invalid Profile Id";
    }
...
if ($type == "DeleteCustomerProfile") {
    ...
    if (!empty($iProfileId)) {
        ...
        $whereUser = " iUserId = '$iUserId'";
        $iAccountIds = getCustomerAccountsDetails($iUserId);
        $NewAccount = $iAccountIds - 1;
        if ($NewAccount == 0) {
            $dataUser['iProfileId'] = 0;
        }
        $dataUser['iAccountId'] = $NewAccount;
        $idU = $obj->MySQLQueryPerform("register_user", $dataUser, 'update', $whereUser);
...
if ($type == "UpdateCustomerProfile") {
    ...
    $iProfileId = $_REQUEST['iProfileId'] ? $_REQUEST['iProfileId'] : '';
    $iAccountId = isset($_REQUEST["iAccountId"]) ? $_REQUEST["iAccountId"] : '';
    // $accttyppe = $_REQUEST['accttyppe'] ? $_REQUEST['accttyppe'] : '';
    $account = $_REQUEST['account'] ? $_REQUEST['account'] : '';
    $expiry = $_REQUEST['expiry'] ? $_REQUEST['expiry'] : '';
    $cvv2 = $_REQUEST['cvv2'] ? $_REQUEST['cvv2'] : '';
    $postal = $_REQUEST['postal'] ? $_REQUEST['postal'] : '';
    $profileupdate = $_REQUEST['profileupdate'] ? $_REQUEST['profileupdate'] : '';
    $defaultacct = $_REQUEST['defaultacct'] ? $_REQUEST['defaultacct'] : '';
    // $license = $_REQUEST['license'] ? $_REQUEST['license'] : '123';
    //  $ssnl4 = $_REQUEST['ssnl4'] ? $_REQUEST['ssnl4'] : '123';
    $userData = getRequiredCustomerDetailsFromId($iUserId);
    $ValidExpiry = strlen($expiry);
    if ($ValidExpiry == 5) {
        $expiry = '0' . $expiry;
    }
    if ($ValidExpiry == 3) {
    ...
    $request = array(
        "profile" => "$iProfileId/$iAccountId",
        "profileupdate" => $profileupdate,
        "region" => "NC",
        "phone" => $userData['vPhone'] ? $userData['vPhone'] : " ",
        // "accttype" => $accttyppe,
        "postal" => $postal,
        // "ssnl4"=>$ssnl4,
        ...
        "city" => "Jacksonville",
        "country" => "US",
        "address" => $userData['tDestinationAddress'] ? $userData['tDestinationAddress'] : " ",
        "merchid" => $Merchid,
    ...
    if ($ePaymentOption == "Card") {
        ...
        try {
            ...
            if ($fNetTotal > 0) {
                $CCDetails = getCardConnectDetails();
                $Merchid = $CCDetails['CARDCONNECT_MERCHANTID'];
                $CCUrl = $CCDetails['CARDCONNECT_URL'] . 'auth';
                $CCUsername = $CCDetails['CARDCONNECT_USERNAME'];
                $CCpassword = $CCDetails['CARDCONNECT_PASSWORD'];
                ...
                $request = array(
                    ...
                    'orderid' => $iOrderId,
                    'name' => $userData['name'],
                    'address' => $userData['tDestinationAddress'],
                    'city' => "Jacksonville",
                    'region' => "NC",
                    'country' => "US",
                    'postal' => $postal ? $postal : "",
                    'phone' => $userData['vPhone'] ? $userData['vPhone'] : " ",
                    'email' => $userData['vEmail'] ? $userData['vEmail'] : " ",
                    'ecomind' => 'E',
                    'tokenize' => "Y",
                    'profile' => "Y",
                    'capture' => "Y"
                );
//$userData['vCountry'] ? $userData['vCountry'] :
...
//                $result = get_object_vars($details);
            ...
            if ($fNetTotal == 0 || $response['resptext'] == 'Approval') {
                $where = " iOrderId = '$iOrderId'";
                $data['iTransactionId'] = ($fNetTotal == 0) ? "" : $response['retref'];
                $data['ePaid'] = "Yes";
                $data['gatewayused'] = "CardConnect";
                $data['iStatusCode'] = 1;
                $data['fNetTotal'] = $fNetTotal;
                $data['fWalletDebit'] = $user_wallet_debit_amount;
                //new code for time
                ...
                $whereUser = " iUserId = '$iUserId'";
                $dataUser['vZip'] = $postal;
                $dataUser['iProfileId'] = $response['profileid'];
                $dataUser['iAccountId'] = $response['acctid'];
                $idU = $obj->MySQLQueryPerform("register_user", $dataUser, 'update', $whereUser);
                //Update User Details
                $OrderLogId = createOrderLog($iOrderId, "1");
                $returnArr["Action"] = "1";
                ...
                $OrderLogId = createOrderLog($iOrderId, "11");
                $returnArr['Action'] = "0";
                $returnArr['message'] = "Charge collection failed. Reason : " . $response['resptext'];
                $returnArr['message2'] = $response['resptext'];
...
if ($type == "ChargeByCustomerProfile") {
    $iOrderId = isset($_REQUEST["iOrderId"]) ? $_REQUEST["iOrderId"] : '';
    $iProfileId = isset($_REQUEST["iProfileId"]) ? $_REQUEST["iProfileId"] : '';
    $iAccountId = isset($_REQUEST["iAccountId"]) ? $_REQUEST["iAccountId"] : '';
    $ePaymentOption = isset($_REQUEST["ePaymentOption"]) ? $_REQUEST["ePaymentOption"] : '';
    $sql = "select * from orders where iOrderId='" . $iOrderId . "'";
    $data_order = $obj->MySQLSelect($sql);
...
############################## Capture Card Paymant of Order and Process With Customerid ####################################################################
if ($type == "DriverPosttipFromUser") {
    $iOrderId = isset($_REQUEST["iOrderId"]) ? $_REQUEST["iOrderId"] : '';
    $ePaymentOption = isset($_REQUEST["ePaymentOption"]) ? $_REQUEST["ePaymentOption"] : '';
    $iProfileId = isset($_REQUEST["iProfileId"]) ? $_REQUEST["iProfileId"] : '';
    $iAccountId = isset($_REQUEST["iAccountId"]) ? $_REQUEST["iAccountId"] : '1';
    $CheckUserWallet = isset($_REQUEST["CheckUserWallet"]) ? $_REQUEST["CheckUserWallet"] : 'No';
    $amount = isset($_REQUEST["amount"]) ? $_REQUEST["amount"] : '0';
    $posttipvalue = isset($_REQUEST["posttipvalue"]) ? $_REQUEST["posttipvalue"] : '';
    $image_name = $vImage = isset($_FILES['vImage']['name']) ? $_FILES['vImage']['name'] : '';
    ...
    $vImageName = '';
    if ($iProfileId == '') {
        $GetOrderQ = "SELECT `orders`.`iOrderId`, `orders`.`iUserId`, `register_user`.`iProfileId`, `register_user`.`vEmail`,`register_user`.`vPhone` FROM `orders` JOIN `register_user` ON `register_user`.`iUserId` = `orders`.`iUserId` WHERE `orders`.`iOrderId` = '$iOrderId';";
        $orders = $obj->MySQLSelect($GetOrderQ);
        ...
        if (empty($orders)) {
            ...
            $iProfileId = $orders[0]['iProfileId'];
        }
    }
            ...
            if ($ePaymentOption == "Card") {
                ...
                try {
                    if ($amount > 0) {
                        ...
                        $CCDetails = getCardConnectDetails();
                        $Merchid = $CCDetails['CARDCONNECT_MERCHANTID'];
                        $CCUrl = $CCDetails['CARDCONNECT_URL'] . '';
                        $CCUsername = $CCDetails['CARDCONNECT_USERNAME'];
                        $CCPassword = $CCDetails['CARDCONNECT_PASSWORD'];
                        $request = array(
                            'merchid' => $Merchid,
                        ...
                        $CCurl = $CCDetails['CARDCONNECT_URL'] . "auth";
                        // Prepare new cURL resource
                        //'https://fts.cardconnect.com:6443/cardconnect/rest/'
                        $ch = curl_init($CCurl);
                    ...
                    if ($amount == 0 || $response['resptext'] == "Approval") {

                        $where = " iOrderId = '$iOrderId'";
                        $data['ptTransactionId'] = ($amount == 0) ? "" : $response['retref'];
                        $data['posttipvalue'] = $posttipvalue;
                        $data['posttip'] = $amount;
                        $data['vImageSign'] = $vImageName;
                        $id = $obj->MySQLQueryPerform("orders", $data, 'update', $where);
...
##############################Card Connect Apis WorkFlow Close ###################################
/* Driver Invoice Upload for Payment */

if ($type == 'UploadInvoiceDetailsDriver') {

    $iOrderId = isset($_REQUEST["iOrderId"]) ? $_REQUEST["iOrderId"] : '';
    $invoiceamount = isset($_REQUEST["invoiceamount"]) ? $_REQUEST["invoiceamount"] : '0';
    $invoicenumber = isset($_REQUEST["invoicenumber"]) ? $_REQUEST["invoicenumber"] : '0';
    $image_name = $vImage = isset($_FILES['vImage']['name']) ? $_FILES['vImage']['name'] : '';
    $image_object = isset($_FILES['vImage']['tmp_name']) ? $_FILES['vImage']['tmp_name'] : '';
    if ($image_object) {
        ExifCleaning::adjustImageOrientation($image_object);
    ...
    if ($image_name != "") {
        $Photo_Gallery_folder = $tconfig['tsite_upload_order_images_path_invoice'];
        if (!is_dir($Photo_Gallery_folder))
            mkdir($Photo_Gallery_folder, 0777);
        $vFile = $generalobj->fileupload($Photo_Gallery_folder, $image_object, $image_name, $prefix = '', $vaildExt = "bmp,pdf,doc,docx,jpg,jpeg,gif,png,xls,xlsx,csv");
        $vImageName = $vFile[0];
    ...
    if ($amount != 0 && $invoicenumber != '') {
        ...
        $data['vInvoiceImage'] = $vImageName;
        $id = $obj->MySQLQueryPerform("orders", $data, 'update', $where);
        $OrderLogId = createOrderLog($iOrderId, "1");
        $returnArr["Action"] = "1";
        $returnArr['message'] = "Invoice successfully uploaded.";
    } else {
        $returnArr["Action"] = "0";
        $returnArr['message'] = "Invalid Data";
    }
...
if ($type == 'getpushnotifications') {
    ...
    $count = "select count(*) as total_count from pushnotification_log where vReceivedStatus = 'No' and iUserId IN(" . $GeneralMemberId . ", 0) and status != 'deleted'";
    $totalCount = $obj->sql_query($count);



    if ($totalCount[0]['total_count'] != 0) {

        $per_page = 10;
        $TotalPages = ceil($totalCount[0]['total_count'] / $per_page);
        $pagecount = $page - 1;
        $start_limit = $pagecount * $per_page;
        if ($GeneralMemberId != 0) {
            $sqlNotification = "select * from pushnotification_log where vReceivedStatus = 'No' and iUserId IN(" . $GeneralMemberId . ", 0) and status != 'deleted' order by dDateTime DESC limit $start_limit, $per_page";
        } else {
            $sqlNotification = "select * from pushnotification_log where vReceivedStatus = 'No'and iUserId = 0 and status != 'deleted' order by dDateTime DESC limit $start_limit, $per_page";
        }
        $notification_data = $obj->MySQLSelect($sqlNotification);
        ...
        foreach ($notification_data as $key => $val) {
            if ($val['image'] != '') {
                $notification_data[$key]['image'] = $Photo_Gallery_folder . $val['image'];
            } else {
                $notification_data[$key]['image'] = $tconfig["tsite_url"] . "webimages/upload/DefaultImg/offer_logo.png";
                ;
            }
            if ($val['ipoints'] == '') {
                $notification_data[$key]['ipoints'] = "";
            }
            if ($val['vexpireIn'] == '') {
                $notification_data[$key]['vexpireIn'] = "";
            }
            $notification_data[$key]['dDateTime'] = time_Ago(strtotime($val['dDateTime']));
            //echo $notification_data[$key]['image'];
        }

        $notificationCount = $totalCount[0]['total_count'];
        $returnArr["Action"] = "1";
        $returnArr['message'] = "Notification List Found!.";
        $returnArr["totalCount"] = $notificationCount;
        $returnArr["totalPages"] = $TotalPages;
        $returnArr["data"] = $notification_data;
    } else {
        $returnArr["Action"] = "0";
        $returnArr['message'] = "Notification Not Found";
    }
...
/* GET PUSH NOTIFICATION Update STAUTS LIST */

if ($type == 'updatepushnotifications') {
    $returnArr = array();
    global $obj;
    $GeneralMemberId = isset($_REQUEST["GeneralMemberId"]) ? $_REQUEST["GeneralMemberId"] : '';
    $iPushnotificationId = isset($_REQUEST["iPushnotificationId"]) ? $_REQUEST["iPushnotificationId"] : '';
    if ($GeneralMemberId != 0 && $GeneralMemberId != '') {
        $updateQury = "UPDATE pushnotification_log set vReceivedStatus = 'Yes' WHERE iUserId = '" . $GeneralMemberId . "' and iPushnotificationId = '" . $iPushnotificationId . "' ";
        $obj->sql_query($updateQury);
        $returnArr["Action"] = "1";
        $returnArr['message'] = "Notification status updated successfully.";
    } else {
        $returnArr["Action"] = "0";
        $returnArr['message'] = "Invalid Data";
    }
...
if ($type == "PlugPlayPayment") {
    $iOrderId = isset($_REQUEST["iOrderId"]) ? $_REQUEST["iOrderId"] : '';
    $ePaymentOption = isset($_REQUEST["ePaymentOption"]) ? $_REQUEST["ePaymentOption"] : '';
    //PlugPay Details
    $card_name = $_REQUEST['card_name'] ? $_REQUEST['card_name'] : '';
    $card_number = $_REQUEST['card_number'] ? $_REQUEST['card_number'] : '';
    $expiry = $_REQUEST['expiry'] ? $_REQUEST['expiry'] : '';
    $cvv2 = $_REQUEST['cvv22'] ? $_REQUEST['cvv22'] : '';
    //$postal = $_REQUEST['postal'] ? $_REQUEST['postal'] : '';
    //PlugPay Details
    $sql = "select * from orders where iOrderId='" . $iOrderId . "'";
    $data_order = $obj->MySQLSelect($sql);
    ...
    if ($ePaymentOption == "Card") {
        ...
        try {
            ...
            if ($fNetTotal > 0) {
                ...
                $publisher_name = 'hurleyssup';
                $publisher_password = 'H78@9Ur1e91y+123L+y7';
                $request = array(
                    'publisher-name' => $publisher_name,
                    'publisher-password' => $publisher_password,
                    'mode' => 'auth',
                    'orderID' => $vOrderNo,
                    'card-amount' => $fNetTotal,
                    'card-name' => $card_name,
                    'card-number' => $card_number,
                    'card-exp' => $expiry,
                    'card-cvv' => $cvv2,
                    'phone'=>$userData['vPhone'] ? $userData['vPhone'] : " ",
                   // 'username' => $userData['name'] ? $userData['name'] : "kayhurleys",
                   // 'status' => 'active',
                    'email' => $userData['vEmail'] ? $userData['vEmail'] : " ",
                   // 'card-address1' => $userData['tDestinationAddress'] ? $userData['tDestinationAddress'] : "testing",
                  //  'card-address2' => $userData['tDestinationAddress'] ? $userData['tDestinationAddress'] : "testing1",
                   // 'card-city' => $userData['vState'] ? $userData['vState'] : "test",
                   // 'card-state' => $userData['vState'] ? $userData['vState'] : "test",
                   // 'card-zip' => $userData['vZip'] ? $userData['vZip'] : "160040",
                   // 'card-country' => $userData['vZip'] ? $userData['vZip'] : "cayman",
                   // 'notify-email' => "kubbi@anviam.com"
                );

                $payload = preparePostFields($request);
                //echo "3<br/>";
                // Prepare new cURL resource
                $CCUrl = 'https://pay1.plugnpay.com/payment/pnpremote.cgi';
                $ch = curl_init($CCUrl);
            ...
            if ($fNetTotal == 0 || $response['FinalStatus'] == 'success') {
                $where = " iOrderId = '$iOrderId'";
                $data['iTransactionId'] = ($fNetTotal == 0) ? "" : $response['auth_date'];
                $data['ePaid'] = "Yes";
                $data['gatewayused'] = "CardConnect";
                $data['iStatusCode'] = 1;
                $data['fNetTotal'] = $fNetTotal;
                ...
                $OrderLogId = createOrderLog($iOrderId, "11");
                $returnArr['Action'] = "0";
                $returnArr['message'] = "Charge collection failed. Reason : " . $response['auth-msg'];
                $returnArr['message2'] = $response['resptext'];
