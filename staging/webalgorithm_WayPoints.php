...
//                            $totaltime = $previousorderTimeDiff + $MoreWaitingTime + 2;
                                ...
                                if ($count < 2) {
                                    ...
                                    $CheckPickupSatisfy = CheckTimeDiffBtwTwoLocation($PickoneLat, $PickTwoLat, $PickoneLong, $PickTwoLong, $PickOneTime, $PickTwoTime);
                                    $CheckDeliveryWPickupSatisfy = CheckTimeDiffBtwTwoLocation($PickTwoLat, $driverOrderStacks[0]['DestLatitude'], $PickTwoLong, $driverOrderStacks[0]['DestLongitude'], $PickTwoTime, $driverOrderStacks[0]['mattime']);
                                    if (($CheckPickupSatisfy == "yes") && ($CheckDeliveryWPickupSatisfy == "yes")) {
                                ...
                                } //googlewaypoints added here
                                else {
                                    $CheckPickupSatisfy = CheckTimeDiffBtwTwoLocation($PickoneLat, $PickTwoLat, $PickoneLong, $PickTwoLong, $PickOneTime, $PickTwoTime);

                                    if ($CheckPickupSatisfy == "yes") {
                                        $deliveryOPtions = array();
                                        for ($i = 0; $i < $count; $i++) {
                                            $deliveryOPtions[$i] = array(
                                                'lat' => $driverOrderStacks[$i]['DestLatitude'],
...
//                                        $destination = $getOrderAllLatlong['DestLatitude'].', '.$getOrderAllLatlong['DestLongitude'];
                                        ...
                                        endforeach;
echo "ingooglealgo<br/>";
                                        $CHeckRouteWithMat = GetDrivingDistanceRouting($origin, $destination, $Waypoints, $getOrderAllLatlong['cookingtime'], $deliveryOPtions);
echo "<br/>exit";
exit;
                                        if ($CHeckRouteWithMat == 'no') {
                                            $stisfywithmat = 'no';
                                        }
                                    } else {
                                        $stisfywithmat = 'no';
...
//                                $totaltime = $previousorderTimeDiff + $MoreWaitingTime + 2;
                                ...
                                if ($count < 2) {
                                    ...
                                    if ($CheckPickupSatisfy == "yes") {
                                        ...
                                        $origin = array($PickTwoLat, $PickTwoLong);
                                        $destination = array($getOrderAllLatlong['DestLatitude'], $getOrderAllLatlong['DestLongitude']);
...
//                                }
    ...
    public function getbestordermix($picklat, $picklong, $droplat, $droplong) {
    ...
    public function cookingtimecalculator($order = null) {
        global $generalobj, $obj;
    ...
    public function getneareastdriverlistfromdb($picklat, $picklong, $droplat, $droplong) {
...
//$cookingtime = '15';
    ...
    public function millitarybases($passengerLat, $passengerLon, $checkgoogle) {
...
return $objalgo->getLatestOrderDetails();
?>