...
if($eStatus != ''){
 ...
 $eStatussql = " AND mi.eStatus != 'Deleted'";
}
...
$sql = "SELECT COUNT(mi.iMenuItemId) AS Total FROM menu_items as mi LEFT JOIN food_menu f ON f.iFoodMenuId = mi.iFoodMenuId  LEFT JOIN company c ON f.iCompanyId = c.iCompanyId WHERE 1 = 1  $eStatussql $ssql $dri_ssql";
...
$sql = "SELECT mi.*,f.vMenu_".$default_lang.",c.vCompany FROM  `menu_items` as mi LEFT JOIN food_menu f ON f.iFoodMenuId = mi.iFoodMenuId LEFT JOIN company as c on c.iCompanyId=f.iCompanyId  WHERE 1=1 $eStatussql $ssql $dri_ssql $ord LIMIT $start, $per_page";
                ...
                <div class="inner">
                                ...
                                <tr>
                                    ...
                                    <td width="10%" class="padding-right10">
                                      <select name="option" id="option" class="form-control">
                                          ...
                                          <option  value="mi.vItemType_<?=$default_lang?>" <?php if ($option == "mi.vItemType_$default_lang") { echo "selected"; } ?> >Item</option>
										                      <option  value="f.vMenu_<?=$default_lang?>" <?php if ($option == "f.vMenu_$default_lang" || ($menu_itemid !="" && $category_name != "")) { echo "selected"; } ?> >Category</option>
                                          <option  value="c.vCompany" <?php if ($option == "c.vCompany") { echo "selected"; } ?> ><?=$langage_lbl_admin['LBL_ADMIN_RESTAURANT'];?> Name</option>
                                      </select>
                                    </td>
                    ...
                    <div class="table-list">
                        <div class="row">
                            ...
                            <th width="13%"><a href="javascript:void(0);" onClick="Redirect(1,<?php if($sortby == '1'){ echo $order; }else { ?>0<?php } ?>)">Item <?php if ($sortby == 1) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>

                            <th width="13%"><a href="javascript:void(0);" onClick="Redirect(3,<?php if($sortby == '3'){ echo $order; }else { ?>0<?php } ?>)">Item Category <?php if ($sortby == 3) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>
														
														<th width="18%"><a href="javascript:void(0);" onClick="Redirect(2,<?php if($sortby == '2'){ echo $order; }else { ?>0<?php } ?>)"><?php echo $langage_lbl_admin['LBL_RESTAURANT_TXT_ADMIN'];?> <?php if ($sortby == 2) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>

                            <th width="18%" class="align-center">Item Image</th>
														
														<th width="8%"><a href="javascript:void(0);" onClick="Redirect(4,<?php if($sortby == '4'){ echo $order; }else { ?>0<?php } ?>)">Display Order <?php if ($sortby == 4) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>
													...
													?>
														<tr class="gradeA" >
														 <td align="center"><input type="checkbox" id="checkbox" name="checkbox[]" <?php echo $default; ?> value="<?php echo $data_drv[$i]['iMenuItemId']; ?>" />&nbsp;</td>
															<td><?= $generalobjAdmin->clearName($data_drv[$i]['vItemType_'.$default_lang]); ?></td>
                              ...
                              <td align="center">
                                ...
                                <div class="social show-moreOptions for-five openPops_<?= $data_drv[$i]['iMenuItemId']; ?>">
                                    <ul>
                                        ...
                                        <li class="entypo-facebook" data-network="facebook"><a href="javascript:void(0);" onClick="changeStatus('<?php echo $data_drv[$i]['iMenuItemId']; ?>','Inactive')"  data-toggle="tooltip" title="Make Active">
                                            <img src="img/active-icon.png" alt="<?php echo $data_drv[$i]['eStatus']; ?>" >
                                        </a></li>
                                        <li class="entypo-gplus" data-network="gplus"><a href="javascript:void(0);" onClick="changeStatus('<?php echo $data_drv[$i]['iMenuItemId']; ?>','Active')" data-toggle="tooltip" title="Make Inactive">
                                            <img src="img/inactive-icon.png" alt="<?php echo $data_drv[$i]['eStatus']; ?>" >	
                                        </a></li>
                                        <?php if($eStatus != 'Deleted') { ?>
                                        <li class="entypo-gplus" data-network="gplus"><a href="javascript:void(0);" onClick="changeStatusDelete('<?php echo $data_drv[$i]['iMenuItemId']; ?>')"  data-toggle="tooltip" title="Delete">
                                            <img src="img/delete-icon.png" alt="Delete" >
                                        </a></li>
                                        <?php } ?>
                                      <?php  if (SITE_TYPE == 'Demo') { ?>
                                      <li class="entypo-gplus" data-network="gplus"><a href="javascript:void(0);" onClick="resetTripStatus('<?php echo $data_drv[$i]['iMenuItemId']; ?>')"  data-toggle="tooltip" title="Reset">
