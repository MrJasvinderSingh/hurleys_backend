...
//         }
         ...
         if(($val['iStatusCode'] == 2) ){
                ...
                $orderAssignScript .= '<a href="javascript:void(0)" class="custom-order order_button_ocgreen" data-toggle="modal" data-id="'.$val["iOrderId"].'" data-status="'.$val["iStatusCode"].'" data-olddriver="'.$OldDriverId.'" >AS</a>';

            }
            elseif(($val['iStatusCode'] == 4) || ($val['iStatusCode'] == 5 )){
                            $OldDriverId = $val["iDriverId"] ? $val["iDriverId"] : '0';
                $orderAssignScript .= '<a href="javascript:void(0)" class="custom-order order_button_ocgray" data-toggle="modal" data-id="'.$val["iOrderId"].'" data-status="'.$val["iStatusCode"].'" data-olddriver="'.$OldDriverId.'" >UA</a>';

            }
         
        $orderdata .= '<tr style="'.$cls.'  padding:2px; "><td attr="'.$val['iOrderId'].'" style="  text-align: left; vertical-align: middle;cursor:pointer; font-size: 16px;" class="tdoperationcls">' . substr($val['vOrderNo'], -3) . '-'.substr($val['vCompany'], 0,10 ).'<br/>'.$sector['sector'].'-'.substr(cleanspclchar($UserSelectedAddressArr['UserAddress']), 0,10).'</td><td style="text-align: center; vertical-align: middle; font-size: 16px;">'.$status.'<br/>'.$orderAssignScript.'</td><td style="font-size: 16px;">AGE-'.$Age.' <br/> EDT-'.$ETA.'</td> </tr>';
