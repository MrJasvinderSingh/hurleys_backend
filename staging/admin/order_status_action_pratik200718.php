<?
	...
	$tbl_name 	= 'order_status_pratik20072018';
	$script 	= 'order_status_pratik20072018';
	...
	$iOrderStatusId	= isset($_POST['iOrderStatusId'])?$_POST['iOrderStatusId']:$iOrderStatusId;
	...
	$vsTitle = 'vStatus';
	$$vsTitle  = isset($_POST[$vsTitle])?$_POST[$vsTitle]:'';
	$vsDesc = 'vStatus_Track';
	$$vsDesc  = isset($_POST[$vsDesc])?$_POST[$vsDesc]:'';
	if($count_all > 0) {
	...
	if(isset($_POST['submit'])) { //form submit
	  if(!empty($iOrderStatusId)){
          if(SITE_TYPE=='Demo')
          {
            header("Location:order_status_action_pratik200718.php?id=".$id."&success=2");
            exit;
          }
      }
		...
		$vsTitle = 'vStatus';
		$vsDesc = 'vStatus_Track';
		$sql_str .= $vsTitle." = '".$$vsTitle."',";
		$sql_str .= $vsDesc." = '".$$vsDesc."',";
		if($count_all > 0) {
			for($i=0;$i<$count_all;$i++) {
				$vTitle = 'vStatus_'.$db_master[$i]['vCode'];
				$vDesc = 'vStatus_Track_'.$db_master[$i]['vCode'];
	...
	if($action == 'Edit') { 
		...
		if($count_all > 0) {
			for($i=0;$i<$count_all;$i++) {
				$vsTitle = 'vStatus';
				$$vsTitle  = isset($db_data[0][$vsTitle])?$db_data[0][$vsTitle]:$$vsTitle;
				$vsDesc = 'vStatus_Track';
				$$vsDesc  = isset($db_data[0][$vsDesc])?$db_data[0][$vsDesc]:$$vsDesc;
				$vTitle = 'vStatus_'.$db_master[$i]['vCode'];
		...
		<div id="wrap">
			...
			<div id="content">
				<div class="inner">
					<div class="row">
						<div class="col-lg-12">
							...
							<a href="order_status_pratik200718.php">
               ...
               <?} ?>
							...
							<form method="post" action="" enctype="multipart/form-data" id="order_status_action_pratik200718" name="order_status_action_pratik200718">
								<input type="hidden" name="id" value="<?=$id;?>"/>
								<input type="hidden" name="temp_order" id="temp_order" value="1">
								<input type="hidden" name="vImage_old" value="<?=$vImage?>">
								<input type="hidden" name="backlink" id="backlink" value="order_status_pratik200718.php"/>
								<?php 
								//echo '<pre>';print_r($db_master);exit;
									$eDefault = $db_master[0]['eDefault']; 
									$required = ($eDefault == 'Yes')?'required':'';
								?>
								<div class="row">
									<div class="col-lg-12">
										<label>Status Title <?=$required_msg;?></label>
									</div>
									<div class="col-lg-6">
										<input type="text" class="form-control" name="vStatus"  id="vStatus" value="<?=$$vsTitle;?>" placeholder="Status Title" <?=$required;?>>
									</div>
								</div>
								
								<div class="row">
									<div class="col-lg-12">
										<label>Status Description <?=$required_msg;?></label>
									</div>
									<div class="col-lg-6">
										<textarea name="vStatus_Track"  id="vStatus_Track" class="form-control" rows="5" placeholder="Status Description"><?=$$vsDesc;?></textarea>
									</div>
								</div>
								<?php 
								...
								if($count_all > 0) {
									...
									<div class="col-lg-12">
                                        ...
                                        <a href="order_status_pratik200718.php" class="btn btn-default back_link">Cancel</a>
                                    </div>
									</div>
							</form>
						</div>
