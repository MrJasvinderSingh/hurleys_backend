			...
			if($status == 'Inactive') {
				...
				if(empty($current_active_trip)) {
	            	...
	            	if($SEND_TAXI_EMAIL_ON_CHANGE == 'Yes') {
						$sql23 = "SELECT m.vMake, md.vTitle,rd.vEmail, rd.vName, rd.vLastName FROM driver_vehicle dv, register_driver rd, make m, model md WHERE dv.eStatus != 'Deleted' AND dv.iDriverId = rd.iDriverId AND dv.iModelId = md.iModelId AND dv.iMakeId = m.iMakeId AND dv.iDriverVehicleId = '".$iDriverVehicleId."'";
						$data_email_drv = $obj->MySQLSelect($sql23);
						$maildata['EMAIL'] =$data_email_drv[0]['vEmail'];
						$maildata['NAME'] = $data_email_drv[0]['vName'];
						$maildata['DETAIL']="Your ".$langage_lbl_admin['LBL_TEXI_ADMIN']." ".$data_email_drv[0]['vTitle']." is temporarly ".$status;
	    ...
	    if(empty($current_active_trip)) {	
			...
			if($SEND_TAXI_EMAIL_ON_CHANGE == 'Yes') {
				foreach($checkbox as $iDriverVehicleId){
					$sql23 = "SELECT m.vMake, md.vTitle,rd.vEmail, rd.vName, rd.vLastName FROM driver_vehicle dv, register_driver rd, make m, model mdWHERE dv.eStatus != 'Deleted' AND dv.iDriverId = rd.iDriverId AND dv.iModelId = md.iModelId AND dv.iMakeId = m.iMakeId AND dv.iDriverVehicleId = '".$iDriverVehicleId."'";
					$data_email_drv = $obj->MySQLSelect($sql23);
					$maildata['EMAIL'] =$data_email_drv[0]['vEmail'];
					$maildata['NAME'] = $data_email_drv[0]['vName'];
					$maildata['DETAIL']="Your ".$langage_lbl_admin['LBL_TEXI_ADMIN']." ".$data_email_drv[0]['vTitle']." is temporarily ".$statusVal;
					$generalobj->send_email_user("ACCOUNT_STATUS",$maildata);
				}
			}
