        ...
        if($total_items[0]['FoodItems'] <= 0) {
            ...
            if(!empty($db_oldData)){
               ...
               // $query = "DELETE FROM food_menu WHERE iFoodMenuId = '".$iFoodMenuId."'";
                ...
                $_SESSION['success'] = '1';
                $_SESSION['var_msg'] = 'Food cateogry deleted successfully.';
            }
        } else {
            $_SESSION['success'] = '3';
	...
	if(SITE_TYPE !='Demo') {
        ...
        /*if($status == 'Active' && $total_items_active[0]['FoodItems'] > 0) {*/
            if($status == 'Active'){
                ...
                $obj->sql_query($query);
                $_SESSION['success'] = '1';
                $_SESSION['var_msg'] = 'Food category activated successfully.';
            }
...
*/
        ...
        if($status == 'Inactive' && $total_items_active[0]['FoodItems'] <= 0) {
            ...
            $obj->sql_query($query);
            $_SESSION['success'] = '1';
            $_SESSION['var_msg'] = 'Food Category Inctivated successfully.';
...
if($checkbox != "" && $statusVal != "") {
	if(SITE_TYPE !='Demo'){
		 ...
		 $obj->sql_query($query);
		 $_SESSION['success'] = '1';
		 $_SESSION['var_msg'] = 'Food_menu(s) updated successfully.';
	}
	else{
		$_SESSION['success']=2;
	}
