...
/* general fucntions */
if ($type != "generalConfigData" && $type != "signIn" && $type != "isUserExist" && $type != "signup" && $type != "LoginWithFB" && $type != "sendVerificationSMS" && $type != "countryList" && $type != "changelanguagelabel" && $type != "requestResetPassword" && $type != "ResetPassword" && $type != "UpdateLanguageLabelsValue" && $type != "staticPage" && $type != "sendContactQuery" && $type != "loadAvailableRestaurants" && $type != "getCuisineList" && $type != "loadSearchRestaurants" && $type != "GetRestaurantDetails" && $type != "signup_company" && $type != "GetItemOptionAddonDetails" && $type != "getHomeBanners" && $type != "getBanners" && $type != "getServiceCategories" && $type != "CheckOutOrderEstimateDetails" && $type != "getFAQ" && $type != "getUserLanguagesAsPerServiceType" && $type != "findmilitarybase" && $type != "findmilitarybuilding" && $type != "getmilitarylocation" && $type != 'getallmilitarylocation' && $type != 'checkisbase' && $type != 'CheckUserOnBase' && $type != 'GetLiveTaskDetailDriverMultiRouting' && $type != "AllDriverRequests" && $type != "AllOrdertimeOrderWebOrdering" && $type != 'ChecllAllTesting' && $type != 'getpushnotifications' && $type != 'PlugPlayPayment' && $type != 'getlatestpoints' && $type != 'uploadInvoicePdf') {
...
if ($type == "CheckOutOrderEstimateDetails") {     //echo "<pre>";print_r($_REQUEST);exit;
	...
	if($iCompanyId == 46){
		...
		$period = new DatePeriod(
			...
			new DateInterval('P1D'), // Define the intervals as Periods of 1 Day
			0 // Apply the interval 6 times on top of the starting date
		);
    ...
    if (!empty($OrderDetails)) {
        ...
        $fDeliveryCharge = getChowcalldeliverycharge();
...
//                        }
        ...
        if ($UserType == 'Driver') {
            $DriverAllValues = OrderTotalEarningForChowcallDriver($iGeneralUserId, $vConvertFromDate, $vConvertToDate, $UserType, $vTimeZone);
    ...
    if ($ePaymentOption == "Card") {
        ...
        try {
            ...
            if ($fNetTotal > 0) {
                ...
                $publisher_name = 'demohurley';
                $publisher_password = 'H78@9Urley+123Loy';
                $request = array(
...
if ($type == "uploadInvoicePdf") {

    $target_path = "/var/www/html/hurleys/staging/webimages/upload/invoicePdf/";
    $base = isset($_REQUEST['pdf']) ? $_REQUEST['pdf'] : '';
    $name = isset($_REQUEST['name']) ? $_REQUEST['name'] : '';
	
    $time_val = time();
    $pdf_arr = explode(".", $name);
	$pdf_arr_data = explode("_", $pdf_arr[0]);
	$card_number = $pdf_arr_data[0];
	$sql = "SELECT * FROM `register_user` WHERE card_number='$card_number'";
    $row = $obj->MySQLSelect($sql);
	
	//print_r($row);die;
	if(count($row) > 0){
		$fileextension = $pdf_arr[count($pdf_arr) - 1];
		$user_id = $row[0]['iUserId'];
	   ...
	   $target_path .= $user_id . "/";
		...
		if (file_exists($path)) {
			$sql = "SELECT * FROM `user_invoice` WHERE vPdf_name='$name' and user_id = $user_id";
			$row = $obj->MySQLSelect($sql);
			if(count($row) == 0){
				$Data_invoice['user_id'] = (int)$user_id;
				$Data_invoice['vPdf_name'] = $name;
				$Data_invoice['dCreated_at'] = @date("Y-m-d H:i:s");
				$id = $obj->MySQLQueryPerform("user_invoice", $Data_invoice, 'insert');
			}else{
				$id = 1;
			}
			if ($id > 0) {
				$returnArrayImg['Action'] = "SUCCESS";
				$returnArrayImg['PdfName'] = "$name is uploaded successfully";
				echo json_encode($returnArrayImg);
			} else {
				$returnArrayImg['Action'] = "Failed";
				echo json_encode($returnArrayImg);
			}
		} else {
			$returnArrayImg['Action'] = "Failed";
		...
		$returnArrayImg['Status'] = "FAILURE";
		$returnArrayImg['Message'] = 'Card number does not exist';
		echo json_encode($returnArrayImg);
		die;
	}
