<?php
	...
	//	header("Location:login_new.php?action=".$type.""); exit;
        header("Location:http://hurleys.anviam.in:8086/login"); exit;
	}	
	if($_POST['submit']){
		...
		if($tablename !='' && $type_action!='' && $id !=''){			
			if($newpassword == $vPassword){				
				...
				if($type_action == 'driver' ){				
					header("Location:login_new.php?action=driver"); exit;
				}else if($type_action == 'company'){
					header("Location:login_new.php?action=driver?&iscompany=1"); exit;
				}else{
    ...
    <!-- End: Left Menu-->
		...
		<div class="page-contant reset-password">
			<div class="page-contant-inner">
				<h2 class="header-page header-page-new"><?=$langage_lbl['LBL_RESET_PASSWORD_TXT'];?>
				<?if(SITE_TYPE =='Demo'){?>
