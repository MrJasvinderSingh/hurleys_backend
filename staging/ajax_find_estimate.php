...
if($dist_fare != '' && $time_fare != "")
{
    ...
    for($i=0;$i<count($db_vType);$i++){
		...
		$cont .= '<li><label>'.$db_vType[$i]['vVehicleType_'.$_SESSION["sess_lang"]].'<img src="assets/img/question-icon.jpg" alt="" title="'.$langage_lbl['LBL_APPROX_DISTANCE_TXT'].' '.$langage_lbl['LBL_FARE_ESTIMATE_TXT'].'"><b>'.$generalobj->trip_currency($total_fare).'</b></label></li>';		
    }
