...
Class General {
    ...
    function send_email_user($type, $db_rec = '', $newsid = '') {
        ...
        switch ($type) {
			...
			case "MEMBER_REGISTRATION_USER_CSV":
                $to_email = 'loyaltyhurleys@gmail.com';//$db_rec['EMAIL'];
                $key_arr = Array("#NAME#", "#EMAIL#", "#OTP#", "#SOCIALNOTES#", "#MAILFOOTER#");
                $val_arr = Array($db_rec['NAME'], $db_rec['EMAIL'], $db_rec['PASSWORD'], $db_rec['SOCIALNOTES'], $MAIL_FOOTER);
                break;
			...
			case "CUSTOMER_RESET_PASSWORD_OTP":

                $to_email = $db_rec['EMAIL'];
                $key_arr = Array("#NAME#", "#EMAIL#", "#OTP#", "#MailFooter#");
                $val_arr = Array($db_rec['NAME'], $db_rec['EMAIL'], $db_rec['OTP'], $MAIL_FOOTER);
                break;
			...
			case "MANUAL_CANCEL_TRIP_ADMIN":
              ...
              case "MANUAL_CANCEL_TRIP_ADMIN_TO_USER":
                $to_email = $db_rec['vRiderMail'];
                $key_arr = Array("#Driver#","#Rider#","#BookingNo#","#SourceAddress#","#Ddate#","#MailFooter#");
                $val_arr = Array($db_rec['vDriver'],$db_rec['vRider'],$db_rec['vBookingNo'],$db_rec['vSourceAddresss'],$db_rec['dBookingdate'],$MAIL_FOOTER);
                break;
             ...
             case "MANUAL_CANCEL_TRIP_ADMIN_TO_COMPANY":
                ...
                $key_arr = Array("#Driver#","#Company#","#BookingNo#","#SourceAddress#","#Ddate#","#MailFooter#");
                $val_arr = Array($db_rec['vDriver'],$db_rec['vCompany'],$db_rec['vBookingNo'],$db_rec['vSourceAddresss'],$db_rec['dBookingdate'],$MAIL_FOOTER);
                break;
        ...
        if ($_SERVER['HTTP_HOST'] == "192.168.1.131") {
            ...
            $emailsend = $this->send_email_smtp_google($to_email, $NOREPLY_EMAIL, $EMAIL_FROM_NAME, $mailsubject, $tMessage,$replyto);
            // $emailsend = $this->send_email_smtp($to_email, $NOREPLY_EMAIL, $EMAIL_FROM_NAME, $mailsubject, $tMessage,$replyto);
...
//		   }
     ...
     function send_email_smtp_google($to, $from, $fromname, $subject, $body, $replyto, $attachment_path1 = "", $attachment_path2 = "", $pdf_attach = "") {
        ...
        $mail->Username = "apikey";//"loyaltyhurleys@gmail.com";
        $mail->Password = "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";//"Anviam123";
        if ($replyto != "") {
            $mail->AddReplyTo($replyto, "");
        }
		$from = 'loyaltyapp@hurleys.ky';
		$fromname = 'Hurleys loyalty';
    ...
    function send_messages_user($type, $db_rec = '', $newsid = '', $maillanguage='') {
        ...
        switch ($type) {
            ...
            case "COMPANY_SEND_MESSAGE_JOB_CANCEL":               
                $key_arr = Array("#Rider#","#Ddate#","#Bookingtime#","#BookingNo#");
                $val_arr = Array($db_rec['vCompany'],$db_rec['dBookingdate'], $db_rec['dBookingtime'] , $db_rec['vBookingNo']);      
            break;
...
//        } else {
 ...
 //       }
        ...
        foreach ($_GET as $keyy => $vall) {
            if (is_array($vall))
                foreach ($vall as $keyy1 => $vall1)
                    $_GET[$vall][$keyy1] = $this->cleanweb($vall1);
            else
                $_GET[$keyy] = $this->cleanweb($vall);
        }
        foreach ($_REQUEST as $keyy => $vall) {
            if (is_array($vall))
                foreach ($vall as $keyy1 => $vall1)
                    $_REQUEST[$vall][$keyy1] = $this->cleanweb($vall1);
            else
                $_REQUEST[$keyy] = $this->cleanweb($vall);
        }
        foreach ($_POST as $keyy => $vall) {
            if (is_array($vall))
                foreach ($vall as $keyy1 => $vall1)
                    $_POST[$vall][$keyy1] = $this->cleanweb($vall1);
            else
                $_POST[$keyy] = $this->cleanweb($vall);
        }
    }
    ...
    function InsertIntoUserWallet($iUserId, $eUserType, $iBalance, $eType, $iTripId, $eFor, $tDescription, $ePaymentStatus, $dDate,$iOrderId = 0,$iTransactionId = "") {

        global $obj;

        if($iOrderId > 0){
         $sql="delete from user_wallet where iOrderId = '".$iOrderId."' AND iUserId = '".$iUserId."' AND eUserType = '".$eUserType."'";
          $obj->sql_query($sql);                                                                                                                         
          
          $sql = "INSERT INTO `user_wallet` (`iUserId`,`eUserType`,`iBalance`,`eType`,`iTripId`, `eFor`, `tDescription`, `ePaymentStatus`, `dDate`, `iOrderId`, `iTransactionId`) VALUES ('" . $iUserId . "','" . $eUserType . "', '" . $iBalance . "','" . $eType . "', '" . $iTripId . "', '" . $eFor . "', '" . $tDescription . "', '" . $ePaymentStatus . "', '" . $dDate . "', '" . $iOrderId . "', '" . $iTransactionId . "')";
          $result = $obj->MySQLInsert($sql);

        }else{
          $sql = "INSERT INTO `user_wallet` (`iUserId`,`eUserType`,`iBalance`,`eType`,`iTripId`, `eFor`, `tDescription`, `ePaymentStatus`, `dDate`, `iTransactionId`) VALUES ('" . $iUserId . "','" . $eUserType . "', '" . $iBalance . "','" . $eType . "', '" . $iTripId . "', '" . $eFor . "', '" . $tDescription . "', '" . $ePaymentStatus . "', '" . $dDate . "', '" . $iTransactionId . "')";
          $result = $obj->MySQLInsert($sql);
        }
    ...
    function userwalletcurrencyFront($currencyratio, $amount, $currencysymbol) {
        global $obj;
        $parameter = 2;
	...
	function RandomStringINT($limit) {
        $characters = '12345HU6789RL';
        $randstring = '';
        for ($i = 0; $i < $limit; $i++) {
            $randstring .= $characters[rand(0, strlen($characters))];
        }
	...
	function get_value($table, $field_name, $condition_field = '', $condition_value = '', $setParams = '', $directValue = '') {
		...
		$where = ($condition_field != '') ? ' WHERE ' . $this->cleanweb($condition_field) : '';
		$where .= ($where != '' && $condition_value != '') ? ' = "' . $this->cleanweb($condition_value) . '"' : '';
   ...
   function contains($point, $polygon){
    ...
    function getOrderPriceDetailsForWeb($iOrderId,$iMemberId='', $eUserType = '') {
        ...
        $UserData = $this->get_value('register_user', 'concat(vName," ",vLastName) as UserName,vPhone as userPhoneNumber,tDestinationAddress,vLatitude,vLongitude', 'iUserId', $iUserId);
        $UserName = $UserData[0]['UserName'];
        $returnArrData['UserName'] = $UserName;
        $returnArrData['userPhoneNumber'] = $UserData[0]['userPhoneNumber'];
        ...
        $returnArrData['DeliveryAddress'] = $UserData[0]['tDestinationAddress'];
        $returnArrData['vLatitude'] = $UserData[0]['vLatitude'];
        $returnArrData['vLongitude'] = $UserData[0]['vLongitude'];
...
//                }
                ...
                if ($OptionTitle != '' && $addonTitle == '') {
                    $all_data_new[$k]['SubTitle'] = 'Options : '.$OptionTitle;
                } else if ($OptionTitle == '' && $addonTitle != '') {
                    $all_data_new[$k]['SubTitle'] = "Toppings: " .$addonTitle;
                } else if ($OptionTitle != '' && $addonTitle != '') {
                    $all_data_new[$k]['SubTitle'] = 'Options : '.$OptionTitle . " | Toppings: " . $addonTitle;
                } else {
                    $all_data_new[$k]['SubTitle'] = '';
                }
           ...
           // $OrderFareDetailsArrTotal['Offer Amount'] = "-".$vSymbol." ".$fOffersDiscount;
            ...
            //New code pretip
             if ($data_order[0]['fpretip'] > 0) {
                $OrderFareDetailsArr['PreTip'] = $vSymbol." ".$data_order[0]['fpretip'];
            }
            //New code pretip close
            ...
            if($data_order[0]['fTotalGenerateFare'] > 0){
                $OrderFareDetailsArrTotal['Total'] = $vSymbol." ".$fNetTotal;
            }
            
            //New code postip
             if ($data_order[0]['posttip'] > 0) {
                $OrderFareDetailsArrTotal['PostTip'] = $vSymbol." ".$data_order[0]['posttip'];
            }
            //New code posttip close

            if($data_order[0]['fTotalGenerateFare'] > 0){
                $OrderFareDetailsArrTotal[$languageLabelsArr['LBL_TOTAL_BILL_AMOUNT_TXT']] = $vSymbol." ".($fNetTotal + $data_order[0]['posttip']);
            }
...
// Old Code Close for option       
    ...
    function orderemaildataDelivered($iOrderId,$sendTo)
    {
        global $tconfig,$generalobj,$obj, $COPYRIGHT_TEXT, $COMPANY_NAME,$EMAIL_FROM_NAME,$NOREPLY_EMAIL;
        ob_start();
        require_once($tconfig["tpanel_path"]."orderdetails_mail_format.php");
