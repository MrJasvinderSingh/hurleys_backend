<?php
// DEMO LINK http://192.168.1.131/eatcaymanloyalityappdev/translate_page_content.php?vLangCode=fr
//http://192.168.1.131/eatcaymanloyalityappdev/assets/libraries/translation/translate_page_content.php?vLangCode=zh-CN
	...
	if(count($data)>0)
	{
		foreach ($data as $key => $value) {
			...
			if($translateValue == '' && $vAPILangCode != '') {
				...
				$url = 'http://api.mymemory.translated.net/get?q='.urlencode($englishLabelValue).'&de=&langpair=en|'.$vAPILangCode;
			...
			/*if($translateValueDesc == '' && $vAPILangCode != '') {
				...
				echo $urlDesc = 'http://api.mymemory.translated.net/get?q='.urlencode($englishDescValue).'&de=&langpair=en|'.$vAPILangCode;
				echo"<br/>";
